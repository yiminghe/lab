...
function createTableConstructor(db) {
  return makeClassConstructor(
    Table.prototype,
    function Table$$1(name, tableSchema, trans) {
      this.db = db;
      this._tx = trans;
      this.name = name;
            ...
            return coreTable
              ...
              .then(function (values) {
                ...
                return Promise.resolve(
                  ...
                  .then(function () {
                    return (
                      keys$$1.length > offset + count &&
                      nextChunk(offset + limit)
                    );
                  });
              });
          };
...
function createCollectionConstructor(db) {
  return makeClassConstructor(
    ...
    function Collection$$1(whereClause, keyRangeGenerator) {
      this.db = db;
...
function createWhereClauseConstructor(db) {
  return makeClassConstructor(
    ...
    function WhereClause$$1(table, index, orCollection) {
      this.db = db;
      this._ctx = {
        table: table,
...
function createTransactionConstructor(db) {
  return makeClassConstructor(
    ...
    function Transaction$$1(mode, storeNames, dbschema, parent) {
      var _this = this;
      this.db = db;
      this.mode = mode;
...
function getExistingValues(table, req, effectiveKeys) {
  return req.type === 'add'
    ? Promise.resolve(new Array(req.values.length))
      ...
      return new Promise(function (resolve, reject) {
        ...
        var results =
          ...
          __spreadArrays(
            keys$$1
              ? keys$$1
              : getEffectiveKeys(tableSchema.primaryKey, {
                  type: type,
                  keys: keys$$1,
                  values: values,
                }),
        ...
        if (type === 'deleteRange') {
          ...
          if (range.type === 3) req = store.clear();
          else req = store.delete(makeIDBKeyRange(range));
        } else {
          var _a = isAddOrPut
              ? outbound
                ? [values, keys$$1]
...
function createVersionConstructor(db) {
  return makeClassConstructor(
    Version.prototype,
    function Version$$1(versionNumber) {
      this.db = db;
      this._cfg = {
  ...
  return {
    getDatabaseNames: function () {
      return hasDatabasesNative
        ? DexiePromise.resolve(indexedDB.databases()).then(function (infos) {
            return infos
              .map(function (info) {
                return info.name;
              })
    ...
    add: function (name) {
      return (
        !hasDatabasesNative &&
        name !== DBNAMES_DB &&
        dbNamesTable.put({ name: name }).catch(nop)
      );
    },
    remove: function (name) {
      return (
        !hasDatabasesNative &&
        name !== DBNAMES_DB &&
        dbNamesTable.delete(name).catch(nop)
      );
    },
  };
}
            ...
            function _continue(key) {
              key != null
                ? cursor.continue(
                    pad(
                      key,
                      req.reverse ? down.MAX_KEY : down.MIN_KEY,
                      keyTail,
                    ),
                  )
                : req.unique
        ...
        var tableMiddleware = __assign(__assign({}, downTable), {
          mutate: function (req) {
            ...
            function addPutOrDelete(req) {
              ...
              return getExistingValues(downTable, req, keys$$1).then(
                  ...
                  var contexts = keys$$1.map(function (key, i) {
                    ...
                    if (req.type === 'delete') {
                      ...
                      if (additionalChanges_1) {
                        ...
                        Object.keys(additionalChanges_1).forEach(
                          function (keyPath) {
...
props(
  ...
  __assign(__assign({}, fullNameExceptions), {
    ...
    exists: function (name) {
      return new Dexie$1(name, { addons: [] })
        .open()
        .then(function (db) {
          db.close();
          return true;

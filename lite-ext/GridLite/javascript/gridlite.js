...
Ext.ux.GridLite = function (config) {
  ...
  this.tableContainer = Ext.DomHelper.append(
    Ext.get(config.id),
    {
      tag: 'div',
    },
  ...
  for (var i = hsize - 1; i >= 0; i--) {
    if (this.initialConfig.headers[i].show) {
      ...
      heads.unshift({
        tag: 'th',
        cls: 'x-grid3-cell',
        style: this.initialConfig.headers[i].width
          ? 'width:' + this.initialConfig.headers[i].width + ';'
          : '',
        cn: [
          {
            tag: 'div',
  ...
  var tableWidth = this.initialConfig.width
    ? 'width:' + this.initialConfig.width
    : '';

  this.mainTable = Ext.DomHelper.append(
    this.tableContainer,
    {
      tag: 'table',
      cellSpacing: '0',
  ...
  this.tbody = Ext.DomHelper.append(
    this.mainTable,
    {
      tag: 'tbody',
      cls: 'x-grid3-body',
  ...
  var tfootHtml =
    '<tr>' +
    "<td colspan='" +
    (this._showHeads.length + 1) +
    "'>" +
    "<div class='pagingWrap'>" +
    "<table class='pagingTable' cellspacing='0'>" +
    '<tr>' +
    '<td>' +
    this._getButtonHtml({
    ...
    '</td>' +
    '<td>' +
    "<span class='pageCurInfo'>" +
    "第 <input id='" +
    this.initialConfig.id +
    "currentPage' title='按回车跳页' /> 页，共 <span id='" +
    this.initialConfig.id +
    "totalPage'></span> 页" +
    '</span>' +
    '</td>' +
  ...
  this._tfoot = Ext.DomHelper.append(
    this.mainTable,
    {
      tag: 'tfoot',
      html: tfootHtml,
  ...
  /*
			页框输入数字跳转
		*/
  if (Ext.get(this.initialConfig.id + 'currentPage'))
    Ext.get(this.initialConfig.id + 'currentPage').on(
      'keydown',
      function (evt) {
        var k = evt.getKey();
        if (k == Ext.EventObject.ENTER) {
          this.fireEvent(
            'jump',
            this,
            Ext.get(this.initialConfig.id + 'currentPage').getValue(),
          );
        }
      },
      this,
  ...
  this._ddHandlers = [];
  this.mainTable
    .select('thead th div.x-grid3-hd-inner')
    .each(function (el, this_, index) {
      var tools = Ext.DomHelper.append(
        el,
        {
          tag: 'div',
          cls: 'x-grid3-tool',
      ...
      if (grid._showHeads[index - 1] && grid._showHeads[index - 1].sortable) {
        var sortTool = Ext.DomHelper.append(
          tools,
          {
            tag: 'div',
        ...
        sortTool.on(
          'click',
          grid._sortAction.createDelegate(grid, [
            sortTool,
            grid._showHeads[index - 1],
          ]),
        );
      }
      ...
      if (Ext.ux.GridLite.DD) {
        if (index == 0) return;
        var splitTool = Ext.DomHelper.append(
          tools,
          {
            tag: 'div',
  ...
  if (Ext.ux.GridLite.DD) {
    this.resizeMarker = Ext.DomHelper.append(
      this.tableContainer,
      {
        tag: 'div',
  ...
  if (this.initialConfig.pageRecords)
    this.loadData(this.initialConfig.pageRecords);
};
...
Ext.extend(Ext.ux.GridLite, Ext.util.Observable, {
  ...
  _getButtonHtml: function (config) {
    return (
      '<table cellspacing="0" cellpadding="0" border="0" class="x-btn-wrap x-btn x-btn-icon" id="' +
      config.id +
      '" style="width: auto;">' +
      '<tbody>' +
      '<tr>' +
      ...
      '<em unselectable="on">' +
      '<button type="button" hideFocus="hideFocus" class="x-btn-text ' +
      config.cls +
      '" ></button>' +
      '</em>' +
      '</td>' +
  ...
  loadData: function (dataInfo) {
    ...
    if (dataInfo.pageSize !== undefined) {
      this._tfoot.show();
      var endRecord =
        dataInfo.totalRecord > dataInfo.fromNo + dataInfo.pageSize
          ? dataInfo.fromNo + dataInfo.pageSize
          : dataInfo.totalRecord;
    ...
    for (var i = 0; i < dataInfo.datas.length; i++) {
      ...
      if (
        this.groupCol &&
        lastGroupCol != this._getPropertyValue(line, this.groupCol)
      ) {
      ...
      for (var j = 0; j < this.initialConfig.headers.length; j++) {
        ...
        if (this.initialConfig.headers[j].renderer) {
          cell.html = this.initialConfig.headers[j].renderer(
            this._getPropertyValue(line, this.initialConfig.headers[j].id),
            line,
          );
        } else {
          cell.html = this._getPropertyValue(
            line,
            this.initialConfig.headers[j].id,
          );
        }
        tr.cn.push(td);
      }
    ...
    if (dataInfo.pageSize !== undefined) {
      if (Ext.get(this.initialConfig.id + 'currentPage'))
        Ext.get(this.initialConfig.id + 'currentPage').dom.value = currentPage;
      if (Ext.get(this.initialConfig.id + 'totalPage'))
        Ext.get(this.initialConfig.id + 'totalPage').update(totalPage);
      if (Ext.get(this.initialConfig.id + 'pageInfo'))
        Ext.get(this.initialConfig.id + 'pageInfo').update(
          '显示 ' +
            (dataInfo.fromNo + 1) +
            ' - ' +
            endRecord +
  ...
  registerButton: function (buttonId) {
    if (Ext.get(this.initialConfig.id + buttonId)) {
      Ext.get(this.initialConfig.id + buttonId).on(
        'click',
        function (evt) {
          if (buttonId == 'refresh') {
            if (
              Ext.get(this.initialConfig.id + buttonId)
                .child('button')
                .hasClass('loading')
            )
              return;
          ...
          if (
            Ext.get(this.initialConfig.id + buttonId).hasClass(
              'x-item-disabled',
            )
          )
            return;
  ...
  destroy: function () {
    if (this.fireEvent('beforedestroy', this) !== false) {
      ...
      this._destroyComposite(
        this.mainTable.select('thead tr th .x-grid3-tool-sort'),
      );

      this._destroyComposite(
        this.mainTable.select('thead th div.x-grid3-hd-inner'),
      );

      this._destroyComposite(this.mainTable.select('thead th'));

      Ext.destroy(this.title);

      this._destroyComposite(this.tbody.select('tr'));

      Ext.destroy(this.tbody, Ext.get(this.initialConfig.id + 'currentPage'));
...
if (Ext.dd && Ext.dd.DD) {
  Ext.ux.GridLite.DD = function (grid, toolSplit) {
    ...
    Ext.ux.GridLite.DD.superclass.constructor.call(
      this,
      toolSplit.id,
      'GridDD-' + toolSplit.id,
    );
  ...
  Ext.extend(Ext.ux.GridLite.DD, Ext.dd.DD, {
    ...
    startDrag: function (x, y) {
      ...
      var baseHeight =
        this.grid.mainTable.getComputedHeight() -
        this.grid.mainTable.select('tfoot').item(0).getComputedHeight();

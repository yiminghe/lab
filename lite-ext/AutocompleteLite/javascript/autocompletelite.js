...
Ext.ux.AutocompleteLite = function (config) {
  ...
  //提示文字
  if (this.emptyText) {
    this.labelTip = Ext.DomHelper.append(
      this._wrapDiv,
      {
        tag: 'label',
        cls: 'labelTip',
    ...
    xy[0] += 5;
    xy[1] +=
      (this.textField.getComputedHeight() - this.labelTip.getComputedHeight()) /
      2;
    this.labelTip.setXY(xy);
    if (this.textField.getValue()) this.labelTip.hide();
  }

  if (this.triger) {
    var triger = Ext.DomHelper.append(
      this._wrapDiv,
      {
        tag: 'span',
        cls: 'x-form-trigger',
  ...
  this.textField.on(
    ...
    function () {
      ...
      if (!this.textField.getValue() && this.labelTip) this.labelTip.show();
      if (checkTimer) {
  ...
  this._autoCompleteUl = Ext.DomHelper.append(
    document.body,
    {
      tag: 'ul',
...
Ext.extend(Ext.ux.AutocompleteLite, Ext.util.Observable, {
  ...
  docMouseDown: function (evt, t) {
    //提示框滚动条点击,提示点击只隐藏提示
    if (
      !this._autoCompleteDiv.isVisible() ||
      t.id == this.textField.id ||
  ...
  typeAhead: function (suggestion) {
    ...
    if (
      this.textField.dom.createTextRange ||
  ...
  keyboard: function (direction) {
    ...
    else if (direction == 'b' && this.curLi < childs.getCount() - 1)
  ...
  getSuggestions: function (autocompleteLite, query, ahead, allowEmpty) {
    ...
    for (var i = 0; i < this.data.length; i++) {
      if (
        ...
        this.data[i].label.indexOf(query) != -1
      ) {
        x.push(this.data[i]);
      }
    }

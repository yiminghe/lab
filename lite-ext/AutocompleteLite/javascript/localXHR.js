Ext.apply(Ext.lib.Ajax, {
  ...
  getHttpStatus: function (reqObj) {
    ...
    try {
      ...
      statObj.isLocal =
        (!reqObj.status && location.protocol == 'file:') ||
        (Ext.isSafari && reqObj.status === undefined);

      statObj.isOK =
        statObj.isLocal || (statObj.status > 199 && statObj.status < 300);
  ...
  handleTransactionResponse: function (o, callback, isAbort) {
    ...
    if (o.status.isError) {
      ...
      responseObject = Ext.applyIf(
        responseObject || {},
        this.createExceptionObject(
          o.tId,
          callback.argument,
  ...
  createResponseObject: function (o, callbackArg) {
    ...
    try {
      //to catch bad encoding problems here
    ...
    if (o.status.isLocal) {
      o.status.isOK =
        (o.status.status = !!obj.responseText.length ? 200 : 404) == 200;
      ...
      if (
        ...
        try {
          ...
          if (typeof DOMParser == 'undefined') {
            ...
            try {
              //Opera 9 will fail parsing non-XML content, so trap here.
  ...
  makeRequest: function (method, uri, callback, postData, options) {
    ...
    if (!o || o.status.isError) {
      ...
      if (o.options.async) {
        //Timers won't work here as it's a blocking call
        this.handleReadyState(o, callback);
      }
...
Ext.lib.Ajax.forceActiveX =
  document.location.protocol == 'file:'; /* or other true/false mechanism */

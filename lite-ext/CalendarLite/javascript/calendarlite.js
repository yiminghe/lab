...
Ext.ux.CalendarLite = function (config) {
  config = config || {};
  if (!config.id) config.id = 'LiteExt_ID' + Ext.id() + '_';
  ...
  var prevNavi = Ext.DomHelper.append(
    navibar,
    {
      tag: 'a',
      cls: 'leftBtn',
  ...
  var monthYear = Ext.DomHelper.append(
    navibar,
    {
      tag: 'div',
      cls: 'monthYear',
  ...
  this.monthYearBtn = Ext.DomHelper.append(
    monthYear,
    {
      tag: 'button',
      cls: 'btn-arrow',
  ...
  //导航后一月，重新渲染日历
  var nextNavi = Ext.DomHelper.append(
    navibar,
    {
      tag: 'a',
      cls: 'rightBtn',
  ...
  for (var i = 0; i < 7; i++) {
    headerDayWeek.push({
      ...
      html:
        '<span>' +
        Ext.ux.CalendarLite.getShortDayName(i, this.code) +
        '</span>',
    });
  }
  ...
  this.body = Ext.DomHelper.append(
    calendarData,
    {
      tag: 'tbody',
    },
  ...
  var tfoot = Ext.DomHelper.append(
    calendarData,
    {
      tag: 'tfoot',
      cn: [
  ...
  /*
		...
		*/
  if (this.enableTime) {
    this.timeBtn = Ext.DomHelper.append(
      tfoottd,
      {
        tag: 'div',
        cls: 'time-btn-wrap',
  ...
  var todayBtn = Ext.DomHelper.append(
    tfoottd,
    {
      tag: 'button',
  ...
  var cancelBtn = Ext.DomHelper.append(
    tfoottd,
    {
      tag: 'button',
...
Ext.extend(Ext.ux.CalendarLite, Ext.util.Observable, {
  ...
  createYearMonthSelector: function () {
    ...
    tr.push({
      ...
      html:
        '<a href="#" hideFocus="on"> ' +
        Ext.ux.CalendarLite.getShortMonthName(1, this.code) +
        ' </a>',
    });
    ...
    tr.push({
      ...
      html:
        '<a href="#" hideFocus="on"> ' +
        Ext.ux.CalendarLite.getShortMonthName(2, this.code) +
        ' </a>',
    });
    ...
    for (var i = 0; i < 5; i++) {
      ...
      tr.push({
        ...
        html:
          '<a href="#" hideFocus="on"> ' +
          Ext.ux.CalendarLite.getShortMonthName(startMonth++, this.code) +
          '</a>',
      });
    ...
    this.yearMonthSelectorBody = Ext.DomHelper.append(
      yearMonthSelectorTable,
      {
        tag: 'tbody',
        cn: trs,
      },
    ...
    var yearMonthSelectortfoot = Ext.DomHelper.append(
      yearMonthSelectorTable,
      {
        tag: 'tfoot',
        cn: [
    ...
    var yearMonthSelectorOkBtn = Ext.DomHelper.append(
      yearMonthSelectortfoottd,
      {
        tag: 'button',
    ...
    var yearMonthSelectorCancelBtn = Ext.DomHelper.append(
      yearMonthSelectortfoottd,
      {
        tag: 'button',
        html: Ext.ux.CalendarLite.Language[this.code].cancel,
  ...
  createTimeSelector: function () {
    ...
    for (var i = 0; i < 12; i += 2) {
      ...
      timeTr.push({
        ...
        html:
          '<a href="#" hideFocus="on"> ' +
          i +
          Ext.ux.CalendarLite.Language[this.code].minute +
          ' </a>',
      });
      ...
      timeTr.push({
        ...
        html:
          '<a href="#" hideFocus="on"> ' +
          i +
          Ext.ux.CalendarLite.Language[this.code].second +
          ' </a>',
      });
    ...
    this.timeSelector = Ext.DomHelper.append(
      document.body,
      {
        tag: 'div',
        cls: 'yearMonthSelector',
    ...
    buttonOk.on(
      ...
      function () {
        //更新时间，并重新渲染导航日历
        this.naviDate.setHours(
          Number(
            this.hourMode.dom.value == 1
              ? Number(this.hourSelector.dom.value) + 12 == 24
                ? 0
                : Number(this.hourSelector.dom.value) + 12
              : this.hourSelector.dom.value,
          ),
          Number(this.selectedMinute),
          Number(this.selectedSecond),
        );
        this.timeBtn.update(
          this.naviDate.getHours() +
            ':' +
            this.naviDate.getMinutes() +
            ':' +
            this.naviDate.getSeconds(),
        );
    ...
    minutePre.on(
      ...
      function (evt) {
        this.currentMinute =
          this.currentMinute > 0 ? this.currentMinute - 1 : this.currentMinute;
        this.loadTimeData(
          this.currentMinute * 12,
          this.currentSecond * 12,
          true,
        );
        evt.stopEvent();
      },
    ...
    minuteNext.on(
      ...
      function (evt) {
        this.currentMinute =
          this.currentMinute < 4 ? this.currentMinute + 1 : this.currentMinute;
        this.loadTimeData(
          this.currentMinute * 12,
          this.currentSecond * 12,
          true,
    ...
    secondPre.on(
      ...
      function (evt) {
        this.currentSecond =
          this.currentSecond > 0 ? this.currentSecond - 1 : this.currentSecond;
        this.loadTimeData(
          this.currentMinute * 12,
          this.currentSecond * 12,
          true,
    ...
    secondNext.on(
      ...
      function (evt) {
        this.currentSecond =
          this.currentSecond < 4 ? this.currentSecond + 1 : this.currentSecond;
        this.loadTimeData(
          this.currentMinute * 12,
          this.currentSecond * 12,
          true,
  ...
  yearMonthSelect: function (evt) {
    ...
    if (target) {
      ...
      if (target.hasClass('date-mp-year')) {
        ...
        this.yearMonthSelectorBody
          .select('.date-mp-month')
          .removeClass('date-mp-sel');
        this.selectedMonth =
          Ext.ux.CalendarLite.Language[this.code].monthNumbers[
            target.first().dom.innerHTML.trim()
          ];
  ...
  timeSelect: function (evt) {
    ...
    if (target) {
      ...
      if (target.hasClass('date-mp-minute')) {
        ...
        this.selectedMinute = target
          .first()
          .dom.innerHTML.trim()
          .replace(Ext.ux.CalendarLite.Language[this.code].minute, '');
        target.addClass('date-mp-sel');
      } else if (target.hasClass('date-mp-second')) {
        this.timeSelector.select('.date-mp-second').removeClass('date-mp-sel');
        this.selectedSecond = target
          .first()
          .dom.innerHTML.trim()
          .replace(Ext.ux.CalendarLite.Language[this.code].second, '');
        target.addClass('date-mp-sel');
      }
    }
    evt.stopEvent();
  ...
  showYearMonthSelector: function () {
    ...
    this.selectedMonth = this.yearMonthNavi.getMonth() + 1;
    this.loadYearMonthData(this.yearMonthNavi);
    Ext.get(this.yearMonthSelector).setXY(
      Ext.get(this.calendarLiteContainer).getXY(),
    );
  ...
  loadTimeData: function (startMinute, startSecond, keepHour) {
    ...
    var naviMinute =
      this.selectedMinute + Ext.ux.CalendarLite.Language[this.code].minute;
    var naviSecond =
      this.selectedSecond + Ext.ux.CalendarLite.Language[this.code].second;
    ...
    this.timeSelector.select('.date-mp-minute a').each(function (el) {
      ...
      if (el.dom.innerHTML.trim() == naviMinute)
    ...
    this.timeSelector.select('.date-mp-second a').each(function (el) {
      ...
      if (el.dom.innerHTML.trim() == naviSecond)
        el.parent().addClass('date-mp-sel');
      else el.parent().removeClass('date-mp-sel');
    });
  },
  ...
  loadYearMonthData: function (navi) {
    ...
    var naviMonth = Ext.ux.CalendarLite.getShortMonthName(
      this.selectedMonth,
      this.code,
    );
    ...
    this.yearMonthSelectorBody.select('.date-mp-year a').each(function (el) {
      el.update(startYear++);
      if (el.dom.innerHTML.trim() == naviYear)
        el.parent().addClass('date-mp-sel');
      else el.parent().removeClass('date-mp-sel');
    });
    this.yearMonthSelectorBody.select('.date-mp-month a').each(function (el) {
      if (el.dom.innerHTML.trim() == naviMonth)
        el.parent().addClass('date-mp-sel');
      else el.parent().removeClass('date-mp-sel');
    });
  },
  ...
  getElapsedWeek: function (start, end) {
    return Math.floor(
      (end.getTime() - start.getTime()) / (7 * 1000 * 60 * 60 * 24),
    );
  },

  //载入当前月日历，并选中指定日期，如果是今月，高亮今天
  loadData: function (currentMonthDate) {
    this.monthYearBtn.update(
      Ext.ux.CalendarLite.Language[this.code].monthNames[
    ...
    if (this.timeBtn)
      this.timeBtn.update(
        currentMonthDate.getHours() +
          ':' +
          currentMonthDate.getMinutes() +
          ':' +
          currentMonthDate.getSeconds(),
      );
    //当月几天？
    ...
    var current = 1;
    var inThisMonth =
      currentMonthDate.getFullYear() == this.today.getFullYear() &&
      currentMonthDate.getMonth() == this.today.getMonth();
    //当前月的处理，每周新开一行
    while (current <= days) {
      //新的一周开始了
      if (i % 7 == 0) {
  ...
  _checkHide: function (evt, t) {
    if (
      t.id == this.calendarLiteContainer.id ||
      evt.within(this.calendarLiteContainer) ||

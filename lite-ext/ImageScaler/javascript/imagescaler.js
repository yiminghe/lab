...
Ext.ux.ImageScaler = function (config) {
  ...
  this.imageScalerHeight =
    Ext.get(this.image).getComputedHeight() * this.scaler;
  this.scalerViewerWidth =
    Ext.get(this.image).getComputedWidth() * this.viewScale;
  this.scalerViewerHeight =
    Ext.get(this.image).getComputedHeight() * this.viewScale;

  this.scalerViewer = Ext.DomHelper.append(
    document.body,
    {
      tag: 'div',
      cls: 'scalerViewer',
  ...
  this.imageScaler = Ext.DomHelper.append(
    document.body,
    {
      tag: 'div',
      cls: 'resizerScaler',
  ...
  this.saveHandle.on('click', function (evt) {
    alert('尚未实现!');
    evt.stopEvent();
  });
  this.cancelHandle.on(
    'click',
    function (evt) {
      this.hide();
      evt.stopEvent();
  ...
  this.imageScaler.select('a').on('click', function (evt) {
    evt.preventDefault();
  });
  ...
  if (!Ext.ux.ImageScaler.mask) {
    ...
    Ext.ux.ImageScaler.mask.on('click', function (evt) {
      this.dom.focus();
      evt.stopEvent();
    });
  }
...
Ext.extend(Ext.ux.ImageScaler, Ext.util.Observable, {
  ...
  cropMouseMove: function (evt) {
    ...
    if (this.resizeCropArea) {
      ...
      width = this.cropAreaStart.width + pointer[0] - this.pointerStart[0];
      height = this.cropAreaStart.height + pointer[1] - this.pointerStart[1];
      ...
      if (true || evt.shiftKey) {
        if (widthPercent > heightPercent) {
          ...
          height = Math.ceil(this.cropAreaStart.height * heightPercent);
        } else {
          widthPercent = heightPercent;
          width = Math.ceil(this.cropAreaStart.width * widthPercent);
        }
      }
      // Check if the new position would be out of bounds
      if (this.cropAreaStart.x + width > this.maxX) {
        width = this.maxX - this.cropAreaStart.x;
      } else if (width < 36) {
        width = 36;
      }
      if (this.cropAreaStart.y + height > this.maxY) {
        height = this.maxY - this.cropAreaStart.y;
      } else if (height < 36) {
        height = 36;
      }
      ...
      // Move the crop area
      left = this.cropAreaStart.x + pointer[0] - this.pointerStart[0];
      top = this.cropAreaStart.y + pointer[1] - this.pointerStart[1];
      // Check if the new position would be out of
      // bounds and limit if necessary
      var maxLeft = this.maxX - this.cropAreaStart.width;
      if (left < 0) {
        left = 0;
      } else if (left > maxLeft) {
        left = maxLeft;
      }
      var maxTop = this.maxY - this.cropAreaStart.height;
      top < 0 && (top = 0);
    ...
    this.scalerViewerImg.setSize(
      this.imageScaler.getComputedWidth() * finalP,
      this.imageScaler.getComputedHeight() * finalP,
    );
  ...
  resizeMouseMove: function (evt) {
    ...
    var width = this.resizeeStart.width + pointer[0] - this.pointerStart[0];
    var height = this.resizeeStart.height + pointer[1] - this.pointerStart[1];
  ...
  show: function () {
    ...
    Ext.ux.ImageScaler.mask.setSize(
      ...
      top:
        Math.max(
          ...
          Ext.getBody().getScroll().top +
            ...
            this.scalerViewer.getComputedHeight(),
        ) + 'px',
    });

...
Ext.extend(Ext.ux.MultiTree, Ext.util.Observable, {
  ...
  removeSelectNodeInstantly: function (info) {
    ...
    if (Ext.fly(this.id + 'selectNodes_' + info.key))
      Ext.fly(this.id + 'selectNodes_' + info.key).remove();
    if (Ext.get(this.id + 'dataNodes_' + info.key))
      Ext.get(this.id + 'dataNodes_' + info.key).dom.checked = false;
  },
  /**
   *移除所有的 已选择节点
   */
  ...
  getSelectKeys: function () {
    ...
    for (var i = 0; i < selects_nodes.length; i++)
      if (selects_nodes[i].getAttribute('key'))
        selects.push(selects_nodes[i].getAttribute('key'));
    return selects;
  },
...
*/
  createThirdUl: function (info) {
    ...
    if (info.children)
      for (var i = 0; i < info.children.length; i++) {
        dataNodesSubUlData.cn.push(
          this.createDataSubLi(info.children[i], selectedMap),
        );
      }
    var popup = Ext.DomHelper.append(
      this.bottomNodesContainer,
      dataNodesSubUlData,
      true,
    );
  ...
  create: function (config) {
    ...
    for (var i = 0; i < config.length; i++)
      this.createDataUl(config[i], dataUls);
    bodyData.cn.push({
      tag: 'h3',
      html:
        '提示：' +
        '当您选择箭头下的具体选项时，将会获得更为准确的搜索结果,您最多可以选择' +
        this.limit +
        '个选项 ',
    });
    ...
    this.selectedNodesUl = this.multiTreeWin.body
      .select('ul.selectedNodes')
      .item(0);
    this.dataNodesUls = this.multiTreeWin.body
      .select('div.dataUlSection')
      .item(0);
    this.bottomNodesContainer = Ext.DomHelper.append(
      document.body,
      {
        tag: 'div',
        cls: 'bottomNodes',
    ...
    for (var i = this.selectedNodes.length - 1; i >= 0; i--) {
      if (Ext.get(this.id + 'dataNodes_' + this.selectedNodes[i])) {
        Ext.get(
          this.id + 'dataNodes_' + this.selectedNodes[i],
        ).dom.checked = true;
      }
    }
  },
  ...
  dataOnClick: function (eventExt) {
    ...
    if (!arrow) return;
    this.currentSubMenu =
      Ext.get(this.id + 'level3_' + targetLi.dom.getAttribute('key')) ||
  ...
  dataOnMouseOver: function (eventExt) {
    ...
    //防止子选择重复冒泡到父元素当前
    if (this.currentItem != null && target.dom.id == this.currentItem.id)
      return true;
  ...
  subOnMouseOver: function (eventExt) {
    ...
    if (!target) return true;
    //防止子选择重复冒泡到父元素当前
    if (this.currentSubItem != null && target.dom.id == this.currentSubItem.id)
      return true;
    //移出原先的选择项
  ...
  gatherInfoByKey: function (keys, infos, datas) {
    for (var i = 0; i < datas.length; i++) {
      ...
      if (datas[i].children)
        this.gatherInfoByKey(keys, infos, datas[i].children);
    }
  },

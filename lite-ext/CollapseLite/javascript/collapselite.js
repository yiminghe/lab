...
Ext.ux.CollapseLite = function (config) {
  ...
  var tpl = new Ext.Template(
    ...
    '<strong>{title}</strong>',
    '</div>',
    '<div class="phc_r"><a class=" x-tool layout-collapse-' +
      this.direction +
      '"></a></div>',
    '</div>',
    '</div>',
  ...
  Ext.DomHelper.insertBefore(
    this.el,
    '<div class="collapsed-control" style="display:none;">' +
      '<a class="x-tool layout-collapse-' +
      this._toogle(this.direction) +
      '"></a>',
    +'</div>',
  );
...
Ext.extend(Ext.ux.CollapseLite, Ext.util.Observable, {
  ...
  setHeight: function (h) {
    ...
    if (dh >= 0) this._body.setHeight(dh);
  },

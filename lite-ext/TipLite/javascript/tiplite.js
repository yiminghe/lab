...
Ext.ux.TipLite = function(config) {
    ...
    this.tipLiteContainer = Ext.DomHelper.append(document.body, {
        tag: 'div',
        cls: 'tip-lite',
        style: 'visibility:hidden'
    },
    true);
    var thl = Ext.DomHelper.append(this.tipLiteContainer, {
        tag: 'div',
        cls: 'tip-lite-hl',
        cn: [
        {
    ...
    var closeTool = Ext.DomHelper.append(tht, {
        tag: 'div',
        cls: 'tip-lite-close',
        style: 'display:none'
    },
    ...
    closeTool.on('click',
    function() {
        if (this.tipLiteContainer.isVisible())
        this.fireEvent('hide');
        this.tipLiteContainer.hide();
    },
    this);
    this.title = Ext.DomHelper.append(tht, {
        tag: 'span',
        cls: 'tip-lite-ht-span',
        html: '&nbsp;'
    },
    true);
    var bl = Ext.DomHelper.append(this.tipLiteContainer, {
        tag: 'div',
        cls: 'tip-lite-bl',
        cn: [
        {
...
Ext.extend(Ext.ux.TipLite, Ext.util.Observable, {
    ...
    setWidth: function(width) {
        if (!width) {
            var tsize = Ext.util.TextMetrics.measure(this.title.dom, this.title.dom.innerHTML);
            var bsize = Ext.util.TextMetrics.measure(this.body.dom, this.body.dom.innerHTML);
            width = Math.min(Math.max(tsize.width, bsize.width, 40), this.maxWidth) + 40;
        }
    ...
    onMouseOut: function(evt) {
        ...
        this.fireEvent('hide');
        this.hideTimer = this.tipLiteContainer.hide.defer(this.hideDelay, this.tipLiteContainer);
    }
    ...
    return {
		...
		*/
        ...
        addTip: function(config) {
            ...
            var mo = holder['mouseover'] = tipLite.onMouseOver.createDelegate(tipLite, [config], 0);

  ...
  init: function () {
    ...
    if (Ext.isIE) {
      ...
      this.el.on(
        ...
        function (evt) {
          if (
            container.contains(evt.getRelatedTarget()) ||
            container.dom == evt.getRelatedTarget()
          ) {
          } else {
            this.stopSlider();
          }
  ...
  setUp: {
    ...
    scrollHorizontal: function () {
      this.imageContainer.setStyle({
        width:
          this._getScrollDistance(0, this._totalNum).width +
          this._getScrollDistance(0, 1).width +
          'px',
      });
      this.imageContainer.dom.appendChild(
        this.imageContainer.first().dom.cloneNode(true),
      );
    ...
    scrollVertical: function () {
      ...
      this.imageContainer.setStyle({
        height:
          this._getScrollDistance(0, this._totalNum).height +
          this._getScrollDistance(0, 1).height +
          'px',
      });
      this.imageContainer.dom.appendChild(
        this.imageContainer.first().dom.cloneNode(true),
  ...
  puzzleTo: function (numInt) {
    ...
    for (var i = 0; i < this.animParts[0]; i++) {
      for (var j = 0; j < this.animParts[1]; j++) {
        ...
        var cx = this._randIt(
          destinedLeft - partWidth,
          destinedLeft + partWidth,
        );
        var cy = this._randIt(
          destinedTop - partHeight,
          destinedTop + partHeight,
        );
        //xhtml tag rule no valid ,sorry
        var part = Ext.DomHelper.append(
          curAWrap,
          {
            tag: 'div',
            style: {

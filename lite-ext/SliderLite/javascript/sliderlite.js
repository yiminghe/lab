...
Ext.ux.SliderLite = function (config) {
		...
		if(this._numbers.getCount()!=this._images.getCount()) {
			alert("图片和小标个数不相等!");
			return;
		}
		
		if(this._numbers.getCount()==0 || this._images.getCount()==0) {			
			return;
		}
		
		this._images.enableDisplayMode('block');
		this._images.hide();		
		...
		numUl.on("mouseover",this._mouseover,this,{
			...
			//stopEvent :true,
			delegate :"li",
			buffer:400  //之前已经触发过 container mouseover ,slider 已停止
...
Ext.extend(Ext.ux.SliderLite,Ext.util.Observable,  {	
		...
		_mouseover:function(evt,numEl){
			//console.log("ul li mouseover");
			var numInt =	numEl.sliderNumberIndex;//parseInt(num.innerHTML.trim())-1;			
			this.goTo(numInt);
		},
		...
		_highlight:function(num) {
			this.getImageByIndex(this._currentNum).hide();
		...
		_unHightlight:function(num,callback){			
		    ...
		    endOpacity: 0.2, //can be any value between 0 and 1 (e.g. .5)
		    easing: 'easeOut',
		    duration: .4,
		    callback:callback
			});			

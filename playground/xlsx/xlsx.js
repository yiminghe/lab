...
function xlsx(file) {
  ...
  function convertDate(input) {
    var d = new Date(1900, 0, 0),
      ...
      offset =
        ...
        60000;
    return isDateObject
      ? (input - d - offset) / 86400000 + 1
      : new Date(+d - offset + (input - 1) * 86400000);
  }
  ...
  if (typeof file === 'string') {
    ...
    if ((s = zip.file('xl/sharedStrings.xml'))) {
      ...
      i = s.length;
      while (--i) {
        sharedStrings[i - 1] = unescapeXML(
          s[i].substring(0, s[i].indexOf('</t>')),
        );
    ...
    if ((s = zip.file('docProps/core.xml'))) {
      ...
      s = s.substr(
        s.indexOf('<dcterms:created xsi:type="dcterms:W3CDTF">') + 43,
      );
      result.created = new Date(
        s.substring(0, s.indexOf('</dcterms:created>')),
      );
      s = s.substr(
    ...
    if ((s = zip.file('xl/workbook.xml'))) {
      ...
      i = s.length;
      while (--i) {
        // Do not process i === 0, because s[0] is the text before the first sheet element
    ...
    while (i--) {
      ...
      j = s.length;
      while (--j) {
        // Don't process j === 0, because s[0] is the text before the first row element
        row = w[+getAttr(s[j], 'r') - 1] = [];
        columns = s[j].split('<c ');
        k = columns.length;
        while (--k) {
          // Don't process l === 0, because k[0] is the text before the first c (cell) element
          cell = columns[k];
    ...
    t = t.length
      ? '<numFmts count="' + t.length + '">' + t.join('') + '</numFmts>'
      : '';
    ...
    xl.file(
      ...
      '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">' +
        ...
        (file.activeWorksheet
          ? 'activeTab="' + file.activeWorksheet + '" '
          : '') +

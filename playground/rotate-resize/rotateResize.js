// https://github.com/ioslh/rotate-resize
...
export const EndPoints = class EndPoints {
  ...
  get rt() {
    if (!this._rt) {
      ...
      this._rt = rotatePositionRelatively({ x: x + w, y }, this.center, r);
    }
    return this._rt;
  }
...
const roughlyEqual = (x, y, precision = PRECISION) =>
...
export const computeRectWithPinnedLine = (pinnedPoints, dragPoint, angle) => {
  const [rotateP0, rotateP1] = pinnedPoints.map((p) =>
    rotatePosition(p, -angle),
  );
...
export const squareCrossPoint = (points, point) => {
  ...
  const nextX =
    (baseSlope * p0.x + y - p0.y - verticalSlope * x) /
    (baseSlope - verticalSlope);
...
const resizeRect = (
  ...
  let result;
  if (length === 1) {
    result = computeRectWithCrossPoints(
      pinnedPoints[0],
      fixedMouseEnd,
      rectStart.r,
    );
    ...
    if (fixedRatio) {
      ...
      result = computeRectWithPinnedLine(
        pinnedPoints,
        fixedMouseEnd,
        rectStart.r,
      );
 ...
 */
export default (adjustType, mouseStart, mouseEnd, rectStart, fixedRatio) => {
  if (adjustType === 'move') {

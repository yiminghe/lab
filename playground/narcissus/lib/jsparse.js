...
Narcissus.parser = (function () {
  ...
  StaticContext.prototype = {
    ...
    pushTarget: function (target) {
      ...
      if (this.currentLabels.isEmpty()) {
        return isDefaultTarget ? this.update({ defaultTarget: target }) : this;
      }
    ...
    allow: function (type) {
      switch (type) {
        case EXPORT:
          if (!this.inModule || this.inFunction || !this.topLevel) return false;
        // FALL THROUGH
  ...
  definitions.defineGetter(Np, 'length', function () {
    throw new Error(
      'Node.prototype.length is gone; ' + 'use n.children.length instead',
    );
  });
  ...
  function Statements(t, x, n) {
    try {
      while (!t.done && t.peek(true) !== RIGHT_CURLY) n.push(Statement(t, x));
    } catch (e) {
      if (t.done) t.unexpectedEOF = true;
      throw e;
    }
  }
  ...
  function registerExport(exports, decl) {
    ...
    switch (decl.type) {
      ...
      case EXPORT:
        for (var i = 0; i < decl.pathList.length; i++) {
          ...
          switch (path.type) {
              ...
              for (var j = 0; j < path.children.length; j++) {
                ...
                if (init.type === IDENTIFIER)
                  ...
                  register(
                    ...
                    new Export(init.children[1], false),
                  );
              }
              break;
            ...
            default:
              throw new Error(
                'unexpected export path: ' + definitions.tokens[path.type],
              );
          }
        }
        break;
      ...
      default:
        throw new Error(
          'unexpected export decl: ' + definitions.tokens[exp.type],
        );
    }
  }
  ...
  function Statement(t, x) {
    ...
    if (x.blackList[tt])
      throw t.newSyntaxError(
        definitions.tokens[tt] + ' statements only allowed in Harmony',
      );
    if (!x.allow(tt))
      throw t.newSyntaxError(
        definitions.tokens[tt] + ' statement in illegal context',
      );
    ...
    switch (tt) {
      ...
      case FUNCTION:
        ...
        return FunctionDefinition(
          ...
          x,
          true,
          x.topLevel ? DECLARED_FORM : STATEMENT_FORM,
        );
      ...
      case CONTINUE:
        ...
        n.target = n.label
          ? x2.labeledTargets.find(function (target) {
        ...
        if (!n.target)
          throw t.newSyntaxError(
            'Invalid ' + (tt === BREAK ? 'break' : 'continue'),
          );
      ...
      case TRY:
        ...
        if (t.match(FINALLY)) n.finallyBlock = Block(t, x);
      ...
      case FINALLY:
        throw t.newSyntaxError(
          definitions.tokens[tt] + ' without preceding try',
        );
      ...
      default:
        if (tt === IDENTIFIER) {
          ...
          if (tt === COLON) {
            ...
            n.target =
              n.statement.type === LABEL ? n.statement.target : n.statement;
            return n;
          }
        }
  ...
  function ModuleVariables(t, x, n) {
    ...
    do {
      ...
      if (t.match(ASSIGN)) {
        ...
        n1.initializer = n2;
        if (n2.type === STRING) x.parentScript.modLoads.set(n1.value, n2.value);
        else x.parentScript.modAssns.set(n1.value, n1);
      }
      n.push(n1);
    } while (t.match(COMMA));
  }
  ...
  function LetBlock(t, x, isStatement) {
    ...
    if (isStatement && t.peek() !== LEFT_CURLY) {
       ...
       */
      n2 = new Node(t, { type: SEMICOLON, expression: n });
      isStatement = false;
    }
  ...
  function HeadExpression(t, x) {
    ...
    if (p === END && !n.parenthesized) {
      ...
      if (tt !== LEFT_CURLY && !definitions.isStatementStartCode[tt])
        throw t.newSyntaxError(
          'Unparenthesized head followed by unbraced body',
        );
    }
    return n;
  }
  ...
  function UnaryExpression(t, x) {
    ...
    switch ((tt = t.get(true))) {
      ...
      case PLUS:
      case MINUS:
        if (tt === PLUS) n = new Node(t, { type: UNARY_PLUS });
        else if (tt === MINUS) n = new Node(t, { type: UNARY_MINUS });
        else n = new Node(t);
      ...
      default:
        ...
        if (
          t.tokens[(t.tokenIndex + t.lookahead - 1) & 3].lineno === t.lineno
        ) {
      ...
      case LEFT_CURLY:
        ...
        object_init: if (!t.match(RIGHT_CURLY)) {
          do {
            tt = t.get();
            if (
              (t.token.value === 'get' || t.token.value === 'set') &&
              t.peek() === IDENTIFIER
            ) {
              if (x.ecma3OnlyMode)

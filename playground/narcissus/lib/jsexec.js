...
Narcissus.interpreter = (function () {
  ...
  function isStackOverflow(e) {
    var re =
      /InternalError: (script stack space quota is exhausted|too much recursion)/;
  ...
  var globalBase = {
    ...
    eval: function eval(s) {
      ...
      x2.scope =
        x.version === 'harmony'
          ? { object: new Object(), parent: x.scope }
          : x.scope;
      try {
    ...
    version: function () {
      return ExecutionContext.current.version;
    },
    quit: function () {
      throw END;
    },
  ...
  function wrapNative(name, val) {
    if (!definitions.isNativeCode(val)) return val;
    return Proxy.createFunction(
      ...
      function () {
        ...
        switch (a.length) {
          ...
          default:
            var argStr = '';
            for (var i = 0; i < a.length; i++) argStr += 'a[' + i + '],';
  ...
  gSp.toSource = function () {
    return this.value.toSource();
  };
  gSp.toString = function () {
    return this.value;
  };
  gSp.valueOf = function () {
    return this.value;
  };
  ...
  function putValue(v, w, vn) {
    if (v instanceof Reference) return ((v.base || global)[v.propertyName] = w);
    throw new ReferenceError(
  ...
  function toObject(v, r, rn) {
    ...
    throw rn
      ? new TypeError(message, rn.filename, rn.lineno)
    ...
    if (rhs.type === IDENTIFIER) {
      if (rhs.value === '*') {
        mod.exports.forEach(function (exportID, exp) {
          if (!mod.exportedModules.has(exportID)) bind(exportID, exportID);
        });
      } else {
  ...
  function execute(n, x) {
    ...
    switch (n.type) {
      case MODULE:
        if (n.body) x.result = executeModule(n, x);
        break;
      ...
      case SCRIPT:
        ...
        if (x.thisModule) {
          defineVar = function (obj, prop) {
            ...
            definitions.defineGetterSetter(
              obj,
              prop,
              function () {
      ...
      case BLOCK:
        c = n.children;
        for (i = 0, j = c.length; i < j; i++) execute(c[i], x);
        break;
      ...
      case IF:
        if (getValue(execute(n.condition, x))) execute(n.thenPart, x);
        else if (n.elsePart) execute(n.elsePart, x);
        break;
      ...
      case SWITCH:
        ...
        switch_loop: for (i = 0, j = a.length; ; i++) {
          if (i === j) {
            if (n.defaultIndex >= 0) {
      ...
      case FOR_IN:
        ...
        t =
          (v === null || v === undefined) && !x.ecma3OnlyMode
            ? v
        ...
        try {
          ...
          if (e === THROW && (j = n.catchClauses.length)) {
            ...
            for (i = 0; ; i++) {
              ...
              try {
                if (t.guard && !getValue(execute(t.guard, x))) continue;
                execute(t.block, x);
                break;
              } finally {
                x.scope = x.scope.parent;
          ...
          if (n.finallyBlock) execute(n.finallyBlock, x);
        }
        break;
      ...
      case CONST:
        ...
        for (i = 0, j = c.length; i < j; i++) {
          ...
          for (s = x.scope; s; s = s.parent) {
            if (hasDirectProperty(s.object, t)) break;
          }
      ...
      case SEMICOLON:
        if (n.expression) x.result = getValue(execute(n.expression, x));
        break;
      ...
      case COMMA:
        c = n.children;
        for (i = 0, j = c.length; i < j; i++) v = getValue(execute(c[i], x));
        break;
      ...
      case HOOK:
        c = n.children;
        v = getValue(execute(c[0], x))
          ? getValue(execute(c[1], x))
      ...
      case CALL:
        ...
        if (t instanceof Activation) t = null;
        v = f.__call__(t, a, x);
        break;
      ...
      case ARRAY_INIT:
        ...
        c = n.children;
        for (i = 0, j = c.length; i < j; i++) {
          if (c[i]) v[i] = getValue(execute(c[i], x));
        }
        v.length = j;
        break;
  ...
  function newModule(mod, scope) {
    ...
    // the module instance
    mod.instance = new ModuleInstance(mod, {
      object: new Object(),
      parent: scope,
    });
  ...
  function newFunction(n, x) {
    ...
    var p = Proxy.createFunction(
      handler,
      function () {
        return fobj.__call__(this, arguments, x);
      },
      function () {
        return fobj.__construct__(arguments, x);
      },
    );
    return p;
  }
  ...
  var FOp = (FunctionObject.prototype = {
    ...
    __hasInstance__: function (v) {
      ...
      if (isPrimitive(p)) {
        throw new TypeError(
          "'prototype' property is not an object",
          this.node.filename,
          this.node.lineno,
        );
      }
    ...
    apply: function (t, a) {
      ...
      if (typeof this.__call__ !== 'function') {
        throw new TypeError(
          'Function.prototype.apply called on' + ' uncallable object',
        );
      }

      if (t === undefined || t === null) t = global;
      else if (typeof t !== 'object') t = toObject(t, t);
      ...
      if (a === undefined || a === null) {
        ...
        throw new TypeError(
          'Second argument to Function.prototype.apply' +
  ...
  function thunk(f, x) {
    return function () {
      return f.__call__(this, arguments, x);
    };
  }
    ...
    for (;;) {
      ...
      try {
        ...
        if (e instanceof Error) {
          print(
            (e.filename || 'stdin') + ':' + e.lineNumber + ': ' + e.toString(),
          );
          if (e.stack) printStackTrace(e.stack);
        }
        {

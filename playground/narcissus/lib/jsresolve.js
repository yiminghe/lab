...
Narcissus.resolver = (function () {
  ...
  StaticEnv.prototype = {
    ...
    lookup: function (x) {
      var env = this;
      while (env && !env.frame.has(x)) env = env.parent;
      var def = env && env.frame.get(x);
      if (!def) throw new ReferenceError('unbound reference to ' + x);
      return def;
    },
    ...
    hoistBarrier: function (x) {
      ...
      while (!env.isScriptFrame()) {
        if (env.frame.has(x)) return env.frame.get(x);
      }
      return null;
    },
  ...
  function link(mod) {
    ...
    function linkExport(mod, exp, visited) {
      ...
      if (
        visited.some(function (x) {
          return x.mod === mod2 && x.exp === exp2;
        })
      )
  ...
  function resolveCatch(node, env) {
    ...
    if (node.guard) resolveExpression(node.guard, env2);
  ...
  function bindImports(impDecls, env) {
    for (var i = 0; i < impDecls.length; i++) {
      ...
      for (var j = 0; j < list.length; j++) bindImport(list[j], env);
    }
  }
    ...
    function bind(importID, exportID) {
      if (m.exportedModules.has(exportID))
        throw new ReferenceError(
          "'import' cannot bind modules; use 'module' instead",
        );
      if (!m.exports.has(exportID))
        throw new ReferenceError(
          'export ' + exportID + ' not found in ' + m.displayName,
        );
      env.bindUnique(
        importID,
        new Def(IMPORT_DEF, { module: m, id: exportID }),
      );
    }
  ...
  function pathHead(path) {
    while (path.type === DOT) path = path.children[0];
    return path.value;
  }
  ...
  function bindModuleRedeclarations(modDecls, env) {
    modDecls.forEach(function (name, node) {
      ...
      if (def.type !== MODULE_DEF)
        throw new ReferenceError(
          'attempt to redeclare variable ' + name + ' as a module',
        );
  ...
  function bindModuleAssignments(modAssns, env) {
    ...
    function bindAssn(assn, dependents) {
      ...
      if (!assn.initializer) {
        env.bindUnique(
          localID,
          new Def(MODULE_DEF, resolveModulePath(env, localID)),
        );
        return;
      }
      ...
      if (dependents.has(headID))
        throw new ReferenceError(
          'cyclic dependency in module expression: ' +
      ...
      env.bindUnique(
        localID,
        new Def(MODULE_DEF, resolveModulePath(env, assn.initializer)),
      );
    }
  ...
  function resolveLHS(node, env) {
    ...
    if (def) throw new SyntaxError('assignment to imported variable: ' + def);
  }
    ...
    switch (node.type) {
      ...
      case DELETE:
        switch (node.children[0].type) {
          ...
          case INDEX: {
            var def = resolvePath(node, env);
            if (def) throw new SyntaxError('attempt to delete module import');
            break;
          }
      ...
      case COMMA:
      case LIST:
        for (var i in node.children) resolveExpression(node.children[i], env);
        break;
      ...
      case CALL:
        ...
        if (isDirectEval(node.children[0])) node.staticEnv = env;
      // FALL THROUGH
      ...
      case ARRAY_INIT:
        for (var i in node.children) {
          ...
          if (elt) resolveExpression(elt, env);
        }
        break;
  ...
  function resolvePath(node, env) {
    ...
    if (node.type === DOT) {
      ...
      if (!def || def.type !== MODULE_DEF) return null;
  ...
  function resolveVariables(node, env) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (init) resolveExpression(init, env);
    }
  }
  ...
  function resolveStatement(node, env) {
    switch (node.type) {
      ...
      case RETURN:
        if (node.value) resolveExpression(node.value, env);
        return;

      case IF:
        resolveExpression(node.condition, env);
        resolveStatement(node.thenPart, env);
        if (node.elsePart) resolveStatement(node.elsePart, env);
        return;
      ...
      case SWITCH:
        ...
        for (var i = 0; i < node.cases.length; i++) {
          ...
          if (caseNode.caseLabel) resolveExpression(caseNode.caseLabel, env);
      ...
      case FOR:
        ...
        if (node.condition) resolveExpression(node.condition, env);
        if (node.update) resolveExpression(node.update, env);
        resolveStatement(node.body, env);
        break;

      case FOR_IN:
        if (node.varDecl) resolveVariables(node.varDecl, env);
        else resolveLHS(node.iterator, env);
      ...
      case TRY:
        ...
        if (node.finallyBlock) resolveStatement(node.finallyBlock, env);
        return;

...
Narcissus.definitions = (function () {
  ...
  function getPropertyNames(obj) {
    ...
    while (obj) {
      ...
      for (var i = 0, n = names.length; i < n; i++) table[names[i]] = true;
      obj = Object.getPrototypeOf(obj);
    }
  ...
  function blacklistHandler(target, blacklist) {
    ...
    var redirect = StringMap.create(blacklist).mapObject(function (name) {
      return mask;
    });
  ...
  function whitelistHandler(target, whitelist) {
    ...
    var redirect = StringMap.create(whitelist).mapObject(function (name) {
      return target;
    });
  ...
  function mirrorHandler(target, writable) {
    ...
    handler.defineProperty = function (name, desc) {
      ...
      if (desc.writable !== writable)
        throw new Error(
          'mirror property must ' + (writable ? '' : 'not ') + 'be writable',
        );
  ...
  function mixinHandler(redirect, catchall) {
    ...
    function enumerateMux() {
      ...
      for (name in catchall) {
        if (!hasOwn(redirect, name)) result.push(name);
      }
      return result;
    }
  ...
  StringMap.prototype = {
    ...
    forEach: function (f) {
      ...
      for (var key in table) f.call(this, key, table[key]);
    },
    map: function (f) {
    ...
    copy: function () {
      var table = Object.create(null, {});
      for (var key in this.table) table[key] = this.table[key];
  ...
  function searchMap(map, key, found, notFound) {
    ...
    for (var i = 0, n = a.length; i < n; i++) {
      var pair = a[i];
      if (pair.key === key) return found(pair, i);
    }
    return notFound();
  }

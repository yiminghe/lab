KISSY.Editor.add(
  ...
  function (editor) {
    ...
    var bodyHtml =
        ...
        '</div>',
      footHtml =
        "<div style='padding:5px 20px 20px;'><a class='ke-img-insert ke-button' " +
    ...
    function prepare() {
      ...
      var loadingCancel = new Node(
        "<a class='ke-button' style='position:absolute;" +
          'z-index:' +
      ...
      ok.on('click', function () {
        if (tab.activate() == 'local' && cfg) {
          if (!verifyInputs(commonSettingTable.all('input'))) return;
          ...
          uploadIframe = KE.Utils.doFormUpload(
            {
              ...
              callback: function (r) {
                ...
                var data = S.trim(r.responseText).replace(/\r|\n/g, '');
                d.unloading();
                try {
                  //ie parse error,不抛异常
          ...
          if (!verifyInputs(content.all('input'))) return;
          insert();
        }
      });
    ...
    function update(_selectedEl) {
      ...
      if (selectedEl) {
        ...
        var margin = parseInt(selectedEl._4e_style('margin')) || 0;
        imgMargin.val(margin);
        imgRatio[0].disabled = false;
        imgRatioValue = w / h;
      } else {
        if (tab.getTab('local')) active = 'local';
        resetInput(imgUrl);

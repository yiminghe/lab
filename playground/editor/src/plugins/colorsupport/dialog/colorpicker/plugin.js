KISSY.Editor.add('colorsupport/dialog/colorpicker', function () {
    ...
    function GetColor(color) {
      ...
      if (ret === undefined) {
        ...
        if (document.defaultView) {
          ret = GetData(
            document.defaultView.getComputedStyle(frag, null).color,
          );
        } else {
  ...
  var pickerHtml =
    ...
    map(
      ...
      function (x) {
        return map(
          ColorGrads(['white', 'rgb(' + x.join(',') + ')', 'black'], 5),
          function (x) {
            return (
  ...
  var panelHtml =
    "<div class='ke-color-advanced-picker'>" +
    "<div class='ks-clear'>" +
  ...
  S.augment(ColorPicker, {
    ...
    _detailColor: function (color) {
      ...
      detailPanel.html(
        map(ColorGrads(['#ffffff', color, '#000000'], 40), function (x) {

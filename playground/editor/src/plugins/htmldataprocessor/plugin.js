  ...
  function (editor) {
    ...
    (function () {
      ...
      dtd.parentOf = function (tagName) {
        var result = {};
        for (var tag in this) {
          if (tag.indexOf('$') == -1 && this[tag][tagName]) result[tag] = 1;
        }
        return result;
      };
    })();
    ...
    (function () {
      ...
      function createListBulletMarker(bulletStyle, bulletText) {
        ...
        if (!bulletStyle) {
          ...
          if (!isNaN(bulletStyle[1])) bulletStyle = 'decimal';
          // No way to distinguish between Roman numerals and Alphas,
          // detect them as a whole.
          else if (/^[a-z]+$/.test(bulletStyle[1])) bulletStyle = 'lower-alpha';
          else if (/^[A-Z]+$/.test(bulletStyle[1])) bulletStyle = 'upper-alpha';
          // Simply use decimal for the rest forms of unrepresentable
          // numerals, e.g. Chinese...
          else bulletStyle = 'decimal';

          listType = 'ol';
        } else {
          if (/[l\u00B7\u2002]/.test(bulletStyle[1])) bulletStyle = 'disc';
      ...
      function resolveList(element) {
        ...
        if (
          (listMarker = element.removeAnyChildWithName('ke:listbullet')) &&
          listMarker.length &&
      ...
      function stylesFilter(styles, whitelist) {
        return function (styleText, element) {
          ...
          for (var i = 0; i < rules.length; i++) rules[i] = rules[i].join(':');
      ...
      function assembleList(element) {
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          if ('ke:li' == child.name) {
            ...
            listItemIndent =
              parseInt(listItemAttrs['ke:indent'], 10) ||
              (listBaseIndent &&
                Math.ceil(listItemAttrs['ke:margin'] / listBaseIndent)) ||
              1;
      ...
      var defaultDataFilterRules = {
        ...
        elements: {
          ...
          p: function (element) {
            ...
            if (resolveList(element)) return undefined;
          },
          $: function (el) {
            var tagName = el.name || '';
            //ms world <o:p> 保留内容
      ...
      if (UA.ie) {
        ...
        defaultHtmlFilterRules.attributes.style = function (
          value,
          // , element
        ) {
    ...
    (function () {
      ...
      function extendBlockForDisplay(block) {
        ...
        if (blockNeedsExtension(block)) {
          if (UA.ie) block.add(new KE.HtmlParser.Text('\xa0'));
          else block.add(new KE.HtmlParser.Element('br', {}));
        }
      }
    ...
    var protectAttributeRegex =
        /<((?:a|area|img|input)[\s\S]*?\s)((href|src|name)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+)))([^>]*)>/gi,
    ...
    function protectAttributes(html) {
      return html.replace(
        protectAttributeRegex,
        function (tag, beginning, fullAttr, attrName, end) {
          // We should not rewrite the _cke_saved_src attribute (#5218)
          if (attrName == 'src' && findSavedSrcRegex.test(tag)) return tag;
          else
            return (
              '<' + beginning + fullAttr + ' _ke_saved_' + fullAttr + end + '>'
            );
        },
      );
    }

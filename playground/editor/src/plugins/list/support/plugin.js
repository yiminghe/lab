...
KISSY.Editor.add('list/support', function () {
  ...
  var list = {
    ...
    listToArray: function (
      ...
      if (!listNodeNames[listNode._4e_name()]) return [];
      ...
      for (var i = 0, count = listNode[0].childNodes.length; i < count; i++) {
        ...
        if (listItem._4e_name() != 'li') continue;
        ...
        for (
          var j = 0, itemChildCount = listItem[0].childNodes.length, child;
          j < itemChildCount;
          j++
        ) {
          child = new Node(listItem[0].childNodes[j]);
          if (
            child[0].nodeType == KEN.NODE_ELEMENT &&
    ...
    arrayToList: function (listArray, database, baseIndex, paragraphMode) {
      ...
      while (true) {
        ...
        if (item.indent == indentLevel) {
          ...
          currentListItem = rootNode[0].appendChild(
            item.element._4e_clone(false, true)[0],
          );
          for (var i = 0; i < item.contents.length; i++)
            currentListItem.appendChild(
              item.contents[i]._4e_clone(true, true)[0],
            );
          currentIndex++;
        } else if (item.indent == Math.max(indentLevel, 0) + 1) {
          //进入一个li里面，里面的嵌套li递归构造父亲ul/ol
          var listData = list.arrayToList(
            listArray,
            null,
            currentIndex,
            paragraphMode,
          ...
          if (listNodeNames[item.grandparent._4e_name()]) {
            ...
            } else currentListItem = doc.createDocumentFragment();
          }
          ...
          if (
            currentListItem.nodeType == KEN.NODE_DOCUMENT_FRAGMENT &&
          ...
          if (
            currentListItem.nodeType == KEN.NODE_ELEMENT &&
          ...
          if (
            !UA.ie &&
            (currentListItemName == 'div' || currentListItemName == 'p')
          )
            DOM._4e_appendBogus(currentListItem);
  ...
  ListCommand.prototype = {
    changeListType: function (editor, groupObj, database, listsCreated) {
      ...
      var listArray = list.listToArray(
          groupObj.root,
          database,
          undefined,
          undefined,
      ...
      var fakeParent = new Node(
        groupObj.root[0].ownerDocument.createElement(this.type),
      );
      ...
      var child,
        length = newList.listNode.childNodes.length;
      for (
        i = 0;
        i < length && (child = new Node(newList.listNode.childNodes[i]));
        i++
      ) {
        if (child._4e_name() == this.type) listsCreated.push(child);
      }
    ...
    createList: function (editor, groupObj, listsCreated) {
      ...
      while (listContents.length) {
        ...
        if (!UA.ie) listItem._4e_appendBogus();
      }
      if (insertAnchor[0]) DOM.insertBefore(listNode, insertAnchor);
      else commonParent.append(listNode);
    },
    removeList: function (editor, groupObj, database) {
      ...
      for (var i = 0; i < groupObj.contents.length; i++) {
        var itemNode = groupObj.contents[i];
        itemNode = itemNode._4e_ascendant('li', true);
        if (!itemNode || itemNode.data('list_item_processed')) continue;
        selectedListItems.push(itemNode);
        itemNode._4e_setMarker(database, 'list_item_processed', true);
      }
      ...
      function compensateBrs(isStart) {
        if (
          (boundaryNode = new Node(
            docFragment[isStart ? 'firstChild' : 'lastChild'],
          )) &&
          !(
          ...
          DOM[isStart ? 'insertBefore' : 'insertAfter'](
            editor.document.createElement('br'),
            boundaryNode,
          );
      }
    ...
    exec: function (editor) {
      ...
      while (ranges.length > 0) {
        ...
        while ((block = iterator.getNextParagraph())) {
          ...
          if (block.data('list_block')) continue;
          else block._4e_setMarker(database, 'list_block', 1);
          ...
          for (
            ...
            if (
              listNodeNames[element._4e_name()] &&
              blockLimit.contains(element)
            ) {
      ...
      for (i = 0; i < listsCreated.length; i++) {
        ...
        (mergeSibling = function (rtl) {
          var sibling = listNode[rtl ? '_4e_previous' : '_4e_next'](
            Walker.whitespaces(true),
          );
          if (sibling && sibling[0] && sibling._4e_name() == listCommand.type) {
            sibling._4e_remove();

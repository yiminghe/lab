      ...
      (function () {
        ...
        S.augment(Paste, {
          ...
          _paste: function (ev) {
            ...
            var pastebin = new Node(
              UA.webkit ? '<body></body>' : '<div></div>',
              null,
              doc,
            );
        ...
        var execIECommand = function (editor, command) {
          ...
          (UA.ie > 7 ? doc : doc.selection.createRange())['execCommand'](
            command,
          );
        ...
        var error_types = {
          ...
          paste:
            '您的浏览器安全设置不允许编辑器自动执行粘贴操作，请使用键盘快捷键(Ctrl/Cmd+V)来完成',
        };
        ...
        cutCopyCmd.prototype = {
          exec: function (editor) {
            ...
            if (!success) alert(error_types[this.type]); // Show cutError or copyError.
        ...
        function fixCut(editor) {
          if (!UA.ie || editor.document.compatMode == 'BackCompat') return;

          var sel = editor.getSelection();
          var control;
          if (
            sel.getType() == KES.SELECTION_ELEMENT &&
        ...
        function stateFromNamedCommand(command, doc) {
          ...
          return doc['queryCommandEnabled'](command) ? true : false;
            ...
            (function (cmd) {
              ...
              if (!cmdObj) {
                ...
                cmdObj.on('click', function (ev) {
                  ev.halt();
                  if (cmdObj.hasClass('ke-menuitem-disable')) return;
                  contextmenu.hide();

KISSY.Editor.add('maximize/support', function () {
  var KE = KISSY.Editor,
    ...
    iframe;
  DOM.addStyleSheet(
    '.ke-toolbar-padding {' + 'padding:5px;' + '}',
    'ke-maximize',
  );
  ...
  S.mix(Maximize, {
    ...
    _restoreState: function () {
      ...
      bel.one('span').removeClass(RESTORE_CLASS).addClass(MAXIMIZE_CLASS);
    ...
    _saveSate: function () {
      ...
      var bel = self.btn.get('el');
      bel.one('span').removeClass(MAXIMIZE_CLASS).addClass(RESTORE_CLASS);
    ...
    _real: function () {
      ...
      self._resize =
        self._resize || KE.Utils.buffer(self.cfg._maximize, self, 100);

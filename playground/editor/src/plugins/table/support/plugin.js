  ...
  function () {
      ...
      for (var i = 0; i < ranges.length; i++) {
        ...
        if (range.collapsed) {
          ...
          var startNode = range.getCommonAncestor(),
            nearestCell =
              startNode._4e_ascendant('td', true) ||
          ...
          while ((node = walker.next())) {
            ...
            if (
              parent &&
              cellNodeRegex.test(parent._4e_name()) &&
    ...
    function clearRow($tr) {
      ...
      for (var i = 0; i < $cells.length; i++) {
        $cells[i].innerHTML = '';
        if (!UA.ie) new Node($cells[i])._4e_appendBogus();
      }
    }
    ...
    function deleteRows(selectionOrRow) {
      if (selectionOrRow instanceof KE.Selection) {
        ...
        for (i = rowsToDelete.length; i >= 0; i--) {
          if (rowsToDelete[i]) deleteRows(rowsToDelete[i]);
        }
        ...
        if (table[0].rows.length == 1) table._4e_remove();
        else selectionOrRow._4e_remove();
      }
    ...
    function insertColumn(selection, insertBefore) {
      ...
      var startElement = selection.getStartElement(),
        cell =
          startElement._4e_ascendant('td', true) ||
      ...
      for (var i = 0; i < table[0].rows.length; i++) {
        ...
        if ($row.cells.length < cellIndex + 1) continue;
        cell = new Node($row.cells[cellIndex].cloneNode(false));

        if (!UA.ie) cell._4e_appendBogus();
        // Get back the currently selected cell.
        var baseCell = new Node($row.cells[cellIndex]);
        if (insertBefore) cell.insertBefore(baseCell);
        else cell.insertAfter(baseCell);
      }
    }
    ...
    function deleteColumns(selectionOrCell) {
      if (selectionOrCell instanceof KE.Selection) {
        ...
        for (var i = colsToDelete.length - 1; i >= 0; i--) {
          ...
          if (colsToDelete[i]) deleteColumns(colsToDelete[i]);
        }
    ...
    var contextMenu = {
      ...
      删除表格: function (cmd) {
        ...
        if (
          parent[0].childNodes.length == 1 &&
          parent._4e_name() != 'body' &&
          parent._4e_name() != 'td'
        )
          parent._4e_remove();
        else table._4e_remove();
      },

KISSY.Editor.add('table/dialog/support', function () {
  ...
  DOM.addStyleSheet(
    '.ke-table-config td {' + 'padding:2px;' + '}',
    'ke-table-dialog',
  );
  ...
  S.augment(TableUIDialog, {
    ...
    _modifyTable: function () {
      ...
      if (valid(talignVal)) selectedTable.attr('align', trim(talignVal));
      else selectedTable.removeAttr('align');
      ...
      if (valid(d.tcaption.val())) {
        ...
        if (caption && caption[0]) caption.html(tcv);
        else {
    ...
    _genTable: function () {
      ...
      if (valid(d.tcaption.val())) {
        html +=
          '<caption><span>' +
          KE.Utils.htmlEncode(trim(d.tcaption.val())) +
          '</span></caption>';
      }
      if (d.thead.val()) {
        html += '<thead>';
        html += '<tr>';
        for (i = 0; i < cols; i++) html += '<th>' + cellpad + '</th>';
        html += '</tr>';
        html += '</thead>';
        rows -= 1;
      }
    ...
    _fillTableDialog: function () {
      ...
      d.theight.val(
        (selectedTable._4e_style('height') || '').replace(/px|%/i, ''),
      );
      var c = '';
      if (caption) {
        c = caption.text();
      ...
      d.trows.val(
        selectedTable.one('tbody').children().length +
    ...
    _realTableShow: function () {
      ...
      if (self.selectedTable) {
        ...
        d.get('el').all('.ke-table-create-only').attr('disabled', 'disabled');
        d.thead.disable();
      } else {
        d.get('el').all('.ke-table-create-only').removeAttr('disabled');
        d.thead.enable();
      }
      if (self.selectedTd) {

    ...
    S.augment(MultiUploadDialog, {
      ...
      _prepareShow: function () {
        ...
        var bangpaiUploaderHolder = d.get('body'),
          ...
          listWrap = new Node("<div style='display:none'>").appendTo(
            bangpaiUploaderHolder,
          ),
          btn = new Node(
            "<a class='ke-button'>浏&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;览</a>",
          ).appendTo(btnHolder),
          listTableWrap = new Node(
            '<div>' +
          ...
          fid = S.guid(name),
          statusText = new Node('<span>').prependTo(upHolder);
        ...
        list.on('click', function (ev) {
          ...
          if (target.hasClass('ke-upload-insert')) {
            ...
            var url = tr.attr('url');
            editor.insertElement(
              new Node("<img src='" + url + "'/>", null, editor.document),
            );
          }
          if (
        ...
        if (!UA.webkit && UA.ieEngine != 9) d.set('handlers', [d.get('el')]);
      },
      ...
      _uploadError: function (ev) {
        ...
        if (tr) {
          ...
          tr.one('.ke-upload-progress').html(
      ...
      _syncStatus: function () {
        ...
        if (trs.length == 0) {
          ...
          self.statusText.html(
            '队列中剩余' + wait + '张图片' + '，点击确定上传，开始上传。 ',
          );
        }
        //当前已上传的文件同步到本地
        self._save();
      ...
      _createFileTr: function (tbl, f) {
        ...
        DOM.html(
          cell,
          "<div style='width:160px;overflow:hidden;'><div style='width:9999px;text-align:left;'>" +
            f.name +
            '</div></div>',
        );

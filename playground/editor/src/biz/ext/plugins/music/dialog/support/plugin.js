KISSY.Editor.add('xiami-music/dialog/support', function () {
  ...
  var MIDDLE = 'vertical-align:middle;',
    ...
    bodyHtml =
      ...
      '</div>',
    footHtml =
      "<div style='padding:5px 20px 20px;'><a " +
      ...
      self._xiamia_list.on('click', function (ev) {
        ...
        var t = new Node(ev.target),
          add = t._4e_ascendant(function (node) {
            return (
              self._xiamia_list.contains(node) && node.hasClass('ke-xiami-add')
            );
          }, true),
          paging = t._4e_ascendant(function (node) {
            return (
              self._xiamia_list.contains(node) &&
  ...
  function getDisplayName(r) {
    return (
      decodeURIComponent(r['song_name']) +
      ' - ' +

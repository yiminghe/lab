  ...
  var blockBoundaryDisplayMatch = {
    ...
    editorDom = {
      ...
      _4e_isBlockBoundary: function (el, customNodeNames) {
        el = normalEl(el);
        var nodeNameMatches = S.mix(
          S.mix({}, blockBoundaryNodeNameMatch),
          customNodeNames || {},
        );

        return (
          blockBoundaryDisplayMatch[el.css('display')] ||
      ...
      _4e_isIdentical: function (thisElement, otherElement) {
        if (thisElement._4e_name() != otherElement._4e_name()) return FALSE;
      ...
      _4e_isEmptyInlineRemoveable: function (thisElement) {
        ...
        for (var i = 0, count = children.length; i < count; i++) {
          ...
          if (
            (nodeType == KEN.NODE_ELEMENT &&
              !editorDom._4e_isEmptyInlineRemoveable(child)) ||
      ...
      _4e_mergeSiblings: (function () {
        ...
        function mergeElements(element, sibling, isNext) {
          if (sibling[0] && sibling[0].nodeType == KEN.NODE_ELEMENT) {
            ...
            while (
              sibling.attr('_ke_bookmark') ||
              sibling._4e_isEmptyInlineRemoveable()
            ) {
              pendingNodes.push(sibling);
              sibling = isNext
                ? new Node(sibling[0].nextSibling)
                : new Node(sibling[0].previousSibling);
              if (!sibling[0] || sibling[0].nodeType != KEN.NODE_ELEMENT)
                return;
            }
            ...
            if (element._4e_isIdentical(sibling)) {
              ...
              if (
                innerSibling[0] &&
                innerSibling[0].nodeType == KEN.NODE_ELEMENT
              )
        ...
        return function (thisElement) {
          ...
          mergeElements(
            thisElement,
            new Node(thisElement[0].nextSibling),
            TRUE,
          );
      ...
      _4e_getOffset: function (elem, refDocument) {
        ...
        if (elem[GET_BOUNDING_CLIENT_RECT]) {
          if (elem !== currentDoc.body && currentDocElem !== elem) {
            ...
            x =
              box.left +
              (refDocument === currentDoc
                ? DOM['scrollLeft'](currentWindow)
                : 0);
            y =
              box.top +
              (refDocument === currentDoc
                ? DOM['scrollTop'](currentWindow)
                : 0);
          }
          if (refDocument) {
            ...
            if (currentWindow != refWindow && currentWindow['frameElement']) {
              ...
              var iframePosition = editorDom._4e_getOffset(
      ...
      _4e_clone: function (el, includeChildren, cloneId) {
        ...
        if (!cloneId) {
          var removeIds = function (node) {
            if (node.nodeType != KEN.NODE_ELEMENT) return;
      ...
      _4e_commonAncestor: function (el, node) {
        if (el._4e_equals(node)) return el;

        if (node[0].nodeType != KEN.NODE_TEXT && node.contains(el)) return node;
      ...
      _4e_ascendant: function (el, name, includeSelf) {
        ...
        //到document就完了
        while ($ && $.nodeType != 9) {
          if (!name || name(new Node($)) === TRUE) return new Node($);
              ...
              for (var i = 0; i < attributes.length; i++) {
                ...
                switch (attribute.name) {
                  case 'class':
                    ...
                    if (el.getAttribute('class')) return TRUE;
                    break;
                  /*jsl:fallthru*/
                  default:
      ...
      _4e_position: function (el, otherNode) {
        ...
        if (
          ...
          if ('sourceIndex' in $) {
            return $.sourceIndex < 0 || $other.sourceIndex < 0
      ...
      _4e_setMarker: function (element, database, name, value) {
        ...
        var id =
            element.data('list_marker_id') ||
            element.data('list_marker_id', S.guid()).data('list_marker_id'),
          markerNames =
            element.data('list_marker_names') ||
      ...
      _4e_clearMarkers: function (element, database, removeFromDatabase) {
        ...
        for (var i in names) element.removeData(i);
      ...
      _4e_copyAttributes: function (el, dest, skipAttributes) {
        ...
        // The style:
        if (el.style.cssText !== '') dest[0].style.cssText = el.style.cssText;
      },
      ...
      _4e_getElementsByTagName: function (elem, tag, namespace) {
        ...
        for (var i = 0; i < els.length; i++) re.push(new Node(els[i]));
        return re;
      },
    };

...
KISSY.Editor.add('walker', function (KE) {
  ...
  function iterate(rtl, breakOnFalse) {
    ...
    if (!rtl && !self._.guardLTR) {
      ...
      this._.guardLTR = function (node, movingOut) {
        ...
        return (
          node &&
          node[0] &&
          (!movingOut || !DOM._4e_equals(limitLTR, node)) &&
          //到达深度遍历的最后一个节点，结束
    ...
    if (self.current) node = this.current[getSourceNodeFn](FALSE, type, guard);
    else {
      ...
      if (rtl) {
        ...
        if (range.endOffset > 0) {
          ...
          node =
            guard(node, TRUE) === FALSE
              ? NULL
              : node._4e_previousSourceNode(TRUE, type, guard);
      } else {
        ...
        if (node && node[0]) {
          ...
          node =
            guard(range.startContainer, TRUE) === FALSE
              ? NULL
              : range.startContainer._4e_nextSourceNode(TRUE, type, guard);
      }
    }
    ...
    while (node && node[0] && !self._.end) {
      ...
      } else if (breakOnFalse && self.evaluator) return FALSE;
  ...
  Walker.invisible = function (isReject) {
    ...
    return function (node) {
      ...
      var isInvisible =
        whitespace(node) ||
        (node[0].nodeType == KEN.NODE_ELEMENT && !node[0].offsetHeight);

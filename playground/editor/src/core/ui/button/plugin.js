...
KISSY.Editor.add('button', function () {
  ...
  var TripleButton = S['UIBase'].create(
    [S['UIBase']['Box']],
    {
      bindUI: function () {
        var self = this,
        ...
        el.on('mouseup mouseleave', function () {
          if (self.get('state') == OFF && el.hasClass(ACTIVE_CLASS)) {
            //click 后出发
            setTimeout(function () {
      ...
      _uiSetText: function (text) {
        var self = this,
          el = self.get('el');
        if (text !== undefined) el.html(text);
      },
      ...
      enable: function () {
        var self = this;
        if (self.get('state') == DISABLED) self.set('state', self._savedState);
      },
      _action: function (ev) {
        var self = this;

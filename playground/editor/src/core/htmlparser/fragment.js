...
KISSY.Editor.add('htmlparser-fragment', function () {
  ...
  Fragment.FromHtml = function (fragmentHtml, fixForBody) {
    ...
    function checkPending(newTagName) {
      ...
      if (pendingInline.length > 0) {
        for (var i = 0; i < pendingInline.length; i++) {
          ...
          if (
            (!currentDtd || currentDtd[pendingName]) &&
            (!newTagName ||
              !pendingDtd ||
              pendingDtd[newTagName] ||
              !XHTML_DTD[newTagName])
          ) {
            if (!pendingBRsSent) {
              sendPendingBRs();
              pendingBRsSent = 1;
    ...
    function sendPendingBRs() {
      while (pendingBRs.length) currentNode.add(pendingBRs.shift());
    }
    ...
    function addElement(element, target, enforceCurrent) {
      ...
      if (fixForBody && !target.type) {
        ...
        if (
          element.attributes &&
          (realElementName = element.attributes['_ke_real_element_type'])
        )
        ...
        if (
          ...
          if (enforceCurrent) currentNode = savedCurrent;
        }
      }
    ...
    parser.onTagOpen = function (tagName, attributes, selfClosing) {
      ...
      if (
        currentDtd && // Fragment could receive any elements.
        !element.isUnknown &&
        ...
        if (tagName in listBlocks && currentName in listBlocks) {
          ...
          if (!(lastChild && lastChild.name in listItems))
            //直接添加到父亲
            addElement(
              (lastChild = new KE.HtmlParser.Element('li')),
              currentNode,
            );
          //以后直接跳到父亲不用再向父亲添加
        ...
        // <dd>, specially in IE. Do not enter in this if block in this case.
          ...
          if (nonBreakingBlocks[currentName]) {
            if (!returnPoint) returnPoint = currentNode;
          } else {
            //拆分，闭合掉
        ...
        if (addPoint) currentNode = addPoint;
        // Try adding it to the return point, or the parent element.
        //前面都调用 addElement 将当前节点闭合了，只能往 parent 添加了
        else currentNode = currentNode.returnPoint || currentNode.parent;
    ...
    parser.onTagClose = function (tagName) {
      ...
      if (candidate.type) {
        ...
        if (currentNode.name == 'pre') inPre = FALSE;
      ...
      if (tagName == 'body') fixForBody = FALSE;
    };
  ...
  S.augment(Fragment, {
    ...
    add: function (node) {
      ...
      this._.hasInlineStarted =
        node.type == KEN.NODE_TEXT ||
        (node.type == KEN.NODE_ELEMENT && !node._.isBlockLike);
    },

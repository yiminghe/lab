    ...
    writeHtml: function (writer, filter) {
      ...
      element.filterChildren = function () {
        if (!isChildrenFiltered) {
          ...
          KE.HtmlParser.Fragment.prototype.writeChildrenHtml.call(
            element,
            writer,
            filter,
          );
          element.children = new KE.HtmlParser.Fragment.FromHtml(
            writer.getHtml(),
          ).children;
      ...
      if (filter) {
        while (TRUE) {
          ...
          if (!writeName) {
            this.writeChildrenHtml.call(
              element,
              writer,
              isChildrenFiltered ? NULL : filter,
            );
            return;
          }
        }
      ...
      for (var i = 0; i < 2; i++) {
        for (a in attributes) {
          newAttrName = a;
          value = attributes[a];
          if (i == 1) attribsArray.push([a, value]);
          else if (filter) {
            while (TRUE) {
            ...
            if (newAttrName) {
              if (
                (value = filter.onAttribute(element, newAttrName, value)) ===
                FALSE
              )

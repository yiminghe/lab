...
KISSY.Editor.add('range', function (KE) {
  ...
  KERange.prototype.toString = function () {
    var s = [],
      self = this;
    s.push(
      (self.startContainer[0].id || self.startContainer[0].nodeName) +
        ':' +
        self.startOffset,
    );
    s.push(
      (self.endContainer[0].id || self.endContainer[0].nodeName) +
        ':' +
        self.endOffset,
    );
    return s.join('<br/>');
  ...
  S.augment(KERange, {
    ...
    optimize: function () {
      ...
      if (container[0].nodeType != KEN.NODE_ELEMENT) {
        if (!offset) self.setStartBefore(container);
      ...
      if (container[0].nodeType != KEN.NODE_ELEMENT) {
        if (!offset) self.setEndBefore(container);
        else if (offset >= container[0].nodeValue.length)
    ...
    setStart: function (startNode, startOffset) {
      ...
      var self = this;
      if (
        startNode[0].nodeType == KEN.NODE_ELEMENT &&
    ...
    setStartAt: function (node, position) {
      ...
      switch (position) {
        ...
        case KER.POSITION_BEFORE_END:
          ...
          else self.setStart(node, node[0].childNodes.length);
          break;
    ...
    setEndAt: function (node, position) {
      ...
      switch (position) {
        ...
        case KER.POSITION_BEFORE_END:
          ...
          else self.setEnd(node, node[0].childNodes.length);
          break;
    ...
    execContentsAction: function (action, docFrag) {
      ...
      if (endNode[0].nodeType == KEN.NODE_TEXT)
        ...
        if (endNode[0].childNodes.length > 0) {
          ...
          if (endOffset >= endNode[0].childNodes.length) {
            ...
            removeEndNode = TRUE;
          } else endNode = new Node(endNode[0].childNodes[endOffset]);
        }
      }
      ...
      for (i = 0; i < startParents.length; i++) {
        ...
        // most element.
        if (!topStart._4e_equals(topEnd)) break;
      }
      ...
      for (var j = i; j < startParents.length; j++) {
        ...
        if (clone && !levelStartNode._4e_equals(startNode)) {
          // action = 0 = Delete
        ...
        while (currentNode) {
          ...
          if (
            DOM._4e_equals(endParents[j], currentNode) ||
        ...
        if (levelClone) clone = levelClone;
      }
      ...
      for (var k = i; k < endParents.length; k++) {
        ...
        // For Extract and Clone, we must clone this level.
        if (clone && action > 0 && !levelStartNode._4e_equals(endNode)) {
          // action = 0 = Delete
          levelClone = clone.appendChild(levelStartNode._4e_clone()[0]);
        } else {
          levelClone = null;
        ...
        if (
          ...
          while (currentNode) {
            ...
            if (
              DOM._4e_equals(startParents[k], currentNode) ||
      ...
      if (action == 2) {
        ...
        if (
          startTextNode.nodeType == KEN.NODE_TEXT &&
        ...
        if (
          endTextNode.nodeType == KEN.NODE_TEXT &&
        ...
        if (
          ...
          if (removeStartNode && topEnd.parent()._4e_equals(startNode.parent()))
            endIndex--;
      ...
      if (removeStartNode) startNode._4e_remove();
    ...
    getEnclosedNode: function () {
      ...
      if (
        walkerRange.startContainer[0].nodeType != KEN.NODE_ELEMENT ||
    ...
    shrink: function (mode, selectContents) {
      ...
      if (!self.collapsed) {
        ...
        walker.evaluator = function (node) {
          ...
          return (
            node.nodeType ==
            (mode == KER.SHRINK_ELEMENT ? KEN.NODE_ELEMENT : KEN.NODE_TEXT)
          );
        };
        ...
        walker.guard = function (node, movingOut) {
          ...
          if (movingOut && node == currentElement) return FALSE;
        ...
        if (moveStart) {
          var textStart =
            walker[mode == KER.SHRINK_ELEMENT ? 'lastForward' : 'next']();
          textStart &&
            self.setStartAt(
              textStart,
              selectContents
        ...
        if (moveEnd) {
          walker.reset();
          var textEnd =
            walker[mode == KER.SHRINK_ELEMENT ? 'lastBackward' : 'previous']();
          textEnd &&
            self.setEndAt(
              textEnd,
              selectContents ? KER.POSITION_BEFORE_END : KER.POSITION_AFTER_END,
            );
        }
    ...
    createBookmark2: function (normalized) {
      ...
      if (!startContainer || !endContainer) return { start: 0, end: 0 };
      ...
      if (normalized) {
        ...
        if (startContainer[0].nodeType == KEN.NODE_ELEMENT) {
          ...
          if (
            child &&
            child[0] &&
            child[0].nodeType == KEN.NODE_TEXT &&
            startOffset > 0 &&
        ...
        if (!self.collapsed) {
          ...
          // Normalize the end.
          while (
            endContainer[0].nodeType == KEN.NODE_TEXT &&
    ...
    createBookmark: function (serializable) {
      ...
      if (!collapsed) {
        ...
        if (serializable) endNode.attr('id', baseId + 'E');
      ...
      } else self.moveToPosition(startNode, KER.POSITION_AFTER_END);
    ...
    insertNode: function (node) {
      ...
      if (DOM._4e_equals(node.parent(), self.endContainer)) self.endOffset++;
    ...
    moveToBookmark: function (bookmark) {
      ...
      if (bookmark.is2) {
        ...
        var startContainer = getByAddress(
            self.document,
            bookmark.start,
            bookmark.normalized,
          ),
          ...
          endContainer =
            bookmark.end &&
            getByAddress(self.document, bookmark.end, bookmark.normalized),
        ...
        if (endContainer) self.setEnd(endContainer, endOffset);
        else self.collapse(TRUE);
      } else {
        // Created with createBookmark().
        var serializable = bookmark.serializable,
          startNode = serializable
            ? S.one('#' + bookmark.startNode, self.document)
            : bookmark.startNode,
          endNode = serializable
            ? S.one('#' + bookmark.endNode, self.document)
            : bookmark.endNode;
    ...
    getCommonAncestor: function (includeSelf, ignoreTextNode) {
      ...
      } else ancestor = start._4e_commonAncestor(end);
        ...
        case KER.ENLARGE_ELEMENT:
          ...
          if (container[0].nodeType == KEN.NODE_TEXT) {
            if (offset) {
              ...
              container =
                !S.trim(container[0].nodeValue.substring(0, offset)).length &&
                container;
            ...
            if (offset)
              sibling =
                container[0].childNodes[offset - 1] || container[0].lastChild;
          ...
          while (enlargeable || sibling) {
            if (enlargeable && !sibling) {
              ...
              if (!needsWhiteSpace || enlargeable.css('display') != 'inline') {
                ...
                else self.setStartBefore(enlargeable);
              }
            ...
            while (sibling) {
              ...
              if (sibling.nodeType == KEN.NODE_TEXT) {
                ...
                if (
                  ...
                  if (
                    needsWhiteSpace &&
                    dtd.$removeEmpty[sibling.nodeName.toLowerCase()]
                  ) {
                    ...
                    if (/[^\s\ufeff]/.test(siblingText))
                      ...
                      var allChildren =
                        sibling.all || sibling.getElementsByTagName('*');
              ...
              if (isWhiteSpace) {
                ...
                if (needsWhiteSpace) {
                  if (commonReached) startTop = enlargeable;
                  else if (enlargeable) self.setStartBefore(enlargeable);
          ...
          if (container[0].nodeType == KEN.NODE_TEXT) {
            ...
            // offset. Otherwise, container is NULL.
            container =
              !S.trim(container[0].nodeValue.substring(offset)).length &&
              container;
          ...
          while (enlargeable || sibling) {
            if (enlargeable && !sibling) {
              ...
              if (!needsWhiteSpace || enlargeable.css('display') != 'inline') {
                ...
                else if (enlargeable) self.setEndAfter(enlargeable);
              }
            ...
            while (sibling) {
              ...
              if (isWhiteSpace) {
                if (needsWhiteSpace) {
                  if (commonReached) endTop = enlargeable;
                  else self.setEndAfter(enlargeable);
                }
              }
        ...
        case KER.ENLARGE_LIST_ITEM_CONTENTS:
          ...
          var walker = new Walker(walkerRange),
            ...
            tailBrGuard = function (node) {
              ...
              if (!retval && node[0] && node._4e_name() == 'br') tailBr = node;
              return retval;
            };
          ...
          if (tailBr) self.setEndAfter(tailBr);
      }
    },
    checkStartOfBlock: function () {
      ...
      if (startOffset && startContainer[0].nodeType == KEN.NODE_TEXT) {
        var textBefore = S.trim(
          startContainer[0].nodeValue.substring(0, startOffset),
        );
    ...
    extractContents: function () {
      var self = this,
        docFrag = self.document.createDocumentFragment();
      if (!self.collapsed) self.execContentsAction(1, docFrag);
      return docFrag;
    },
    /**
    ...
    checkBoundaryOfElement: function (element, checkType) {
      ...
      walkerRange[checkType == KER.START ? 'setStartAt' : 'setEndAt'](
        element,
        checkType == KER.START
          ? KER.POSITION_AFTER_START
          : KER.POSITION_BEFORE_END,
      ...
      return walker[
        checkType == KER.START ? 'checkBackward' : 'checkForward'
      ]();
    },
    ...
    getBoundaryNodes: function () {
      ...
      if (startNode[0].nodeType == KEN.NODE_ELEMENT) {
        ...
        else if (childCount < 1) startNode = startNode._4e_previousSourceNode();
      ...
      if (endNode[0].nodeType == KEN.NODE_ELEMENT) {
        ...
        if (childCount > endOffset)
          endNode = new Node(
            endNode[0].childNodes[endOffset],
          )._4e_previousSourceNode(TRUE);
        else if (childCount < 1) endNode = endNode._4e_previousSourceNode();
    ...
    fixBlock: function (isStart, blockTag) {
      ...
      fixedBlock._4e_trim();
      if (!UA.ie) fixedBlock._4e_appendBogus();
    ...
    splitBlock: function (blockTag) {
      ...
      if (!startBlockLimit._4e_equals(endBlockLimit)) return NULL;
      ...
      if (blockTag != 'br') {
        ...
        if (!endBlock) endBlock = self.fixBlock(FALSE, blockTag);
      }
    ...
    moveToElementEditablePosition: function (el, isMoveToEnd) {
      ...
      if (xhtml_dtd.$empty[el._4e_name()]) return FALSE;
      ...
      while (el && el[0].nodeType == KEN.NODE_ELEMENT) {
        ...
        if (isEditable)
          self.moveToPosition(
            el,
            isMoveToEnd ? KER.POSITION_BEFORE_END : KER.POSITION_AFTER_START,
          );
        // Stop immediately if we've found a non editable inline element (e.g <img>).
        else if (xhtml_dtd.$inline[el._4e_name()]) {
          self.moveToPosition(
            el,
            isMoveToEnd ? KER.POSITION_AFTER_END : KER.POSITION_BEFORE_START,
          );
          return TRUE;
        }
        ...
        if (xhtml_dtd.$empty[el._4e_name()])
          el = el[isMoveToEnd ? '_4e_previous' : '_4e_next'](
            nonWhitespaceOrBookmarkEval,
          );
        else
          el = el[isMoveToEnd ? '_4e_last' : '_4e_first'](
            nonWhitespaceOrBookmarkEval,
          );
  ...
  function getCheckStartEndBlockEvalFunction(isStart) {
    var hadBr = FALSE,
      bookmarkEvaluator = Walker.bookmark(TRUE);
    return function (node) {
      ...
      if (node[0].nodeType == KEN.NODE_TEXT) {
        ...
        if (S.trim(node[0].nodeValue).length) return FALSE;
  ...
  function bookmark(contentOnly, isReject) {
    function isBookmarkNode(node) {
      return (
        node && node.nodeName == 'span' && node.getAttribute('_ke_bookmark')
      );
    }
    ...
    return function (node) {
      ...
      // Is bookmark node?
      isBookmark = contentOnly
        ? isBookmark
        : isBookmark || isBookmarkNode(node);
  ...
  function whitespaces(isReject) {
    return function (node) {
      node = node[0] || node;
      var isWhitespace =
        node && node.nodeType == KEN.NODE_TEXT && !S.trim(node.nodeValue);

  ...
  S.augment(Iterator, {
    ...
    getNextParagraph: function (blockTag) {
      ...
      if (!self._.lastNode) {
        ...
        range.enlarge(
          self.forceBrBreak || !self.enlargeBr
            ? KER.ENLARGE_LIST_ITEM_CONTENTS
            : KER.ENLARGE_BLOCK_CONTENTS,
        );
        ...
        if (!self._.lastNode) {
          self._.lastNode = self._.docEndMarker = new Node(
            range.document.createTextNode(''),
          );
      ...
      while (currentNode) {
        ...
        if (!includeNode) {
          ...
          if (currentNode._4e_isBlockBoundary(self.forceBrBreak && { br: 1 })) {
            ...
            if (nodeName == 'br') includeNode = TRUE;
            else if (
              !range &&
            ...
            if (range) {
              ...
              // point. (#1717)
              if (nodeName != 'br') self._.nextNode = currentNode;
            }
        ...
        isLast =
          (!closeRange || includeNode) && currentNode._4e_equals(lastNode);
        ...
        if (range && !closeRange) {
          while (!currentNode[0].nextSibling && !isLast) {
            ...
            if (
              parentNode._4e_isBlockBoundary(self.forceBrBreak && { br: 1 })
            ) {
              closeRange = TRUE;
        ...
        if (includeNode) range.setEndAt(currentNode, KER.POSITION_AFTER_END);
      ...
      if (!block) {
        ...
        if (
          ...
          !block ||
          (self.enforceRealBlocks && block._4e_name() == 'li')
        ) {
          ...
          self._.nextNode = block._4e_equals(lastNode)
            ? NULL
            : range
                .getBoundaryNodes()
                .endNode._4e_nextSourceNode(TRUE, NULL, lastNode);
        }
      }
      ...
      if (removePreviousBr) {
        ...
        if (
          previousSibling[0] &&
          previousSibling[0].nodeType == KEN.NODE_ELEMENT
        ) {
          if (previousSibling._4e_name() == 'br') previousSibling._4e_remove();
          else if (
            previousSibling[0].lastChild &&
            DOM._4e_name(previousSibling[0].lastChild) == 'br'
          )
      ...
      if (removeLastBr) {
        ...
        if (
          lastChild[0] &&
          lastChild[0].nodeType == KEN.NODE_ELEMENT &&
      ...
      if (!self._.nextNode) {
        self._.nextNode =
          isLast || block._4e_equals(lastNode)
            ? NULL

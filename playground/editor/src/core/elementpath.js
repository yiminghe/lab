...
KISSY.Editor.add('elementpath', function (KE) {
  ...
  var checkHasBlock = function (element) {
    ...
    for (var i = 0, count = childNodes.length; i < count; i++) {
      ...
      if (
        child.nodeType == KEN.NODE_ELEMENT &&
  ...
  function ElementPath(lastNode) {
    ...
    while (e && e[0]) {
      if (e[0].nodeType == KEN.NODE_ELEMENT) {
        ...
        if (!blockLimit) {
          if (!block && pathBlockElements[elementName]) block = e;

          if (pathBlockLimitElements[elementName]) {
            // DIV is considered the Block, if no block is available (#525)
            // and if it doesn't contain other blocks.
            if (!block && elementName == 'div' && !checkHasBlock(e)) block = e;
            else blockLimit = e;
          }
        }

        elements.push(e);
        if (elementName == 'body') break;
      }
      e = e.parent();
    }
  ...
  ElementPath.prototype = {
    ...
    compare: function (otherPath) {
      ...
      for (var i = 0; i < thisElements.length; i++) {
        if (!DOM._4e_equals(thisElements[i], otherElements[i])) return FALSE;
      }
    ...
    contains: function (tagNames) {
      ...
      for (var i = 0; i < elements.length; i++) {
        if (elements[i]._4e_name() in tagNames) return elements[i];
      }
      return NULL;
    },
  };

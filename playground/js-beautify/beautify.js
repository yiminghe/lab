...
function js_beautify(js_source_text, options) {
  ...
  // compatibility
  if (
    options.space_after_anon_function !== undefined &&
    options.jslint_happy === undefined
  ) {
    options.jslint_happy = options.space_after_anon_function;
  }
  if (options.braces_on_own_line !== undefined) {
    //graceful handling of depricated option
  ...
  var opt_preserve_newlines =
    typeof options.preserve_newlines === 'undefined'
      ? true
      : options.preserve_newlines;
  var opt_max_preserve_newlines =
    typeof options.max_preserve_newlines === 'undefined'
      ? false
      : options.max_preserve_newlines;
  var opt_jslint_happy =
    options.jslint_happy === 'undefined' ? false : options.jslint_happy;
  ...
  function print_newline(ignore_repeated) {
    ...
    ignore_repeated =
      typeof ignore_repeated === 'undefined' ? true : ignore_repeated;
  ...
  function get_next_token() {
    ...
    if (
      ...
      if (sep === '/') {
        ...
        while (
          parser_pos < input_length &&
          in_array(input.charAt(parser_pos), wordchar)
        ) {
          resulting_string += input.charAt(parser_pos);
          parser_pos += 1;
        }
    ...
    if (c === '#') {
      ...
      if (
        parser_pos < input_length &&
        in_array(input.charAt(parser_pos), digits)
      ) {
        do {
          c = input.charAt(parser_pos);
          sharp += c;
    ...
    if (
      c === '<' &&
      input.substring(parser_pos - 1, parser_pos + 3) === '<!--'
    ) {
      parser_pos += 3;
  ...
  while (
    js_source_text &&
    (js_source_text[0] === ' ' || js_source_text[0] === '\t')
  ) {
      ...
      case 'TK_WORD':
        ...
        if (last_type === 'TK_END_BLOCK') {
          if (
            ...
            if (
              opt_brace_style == 'expand' ||
              opt_brace_style == 'end-expand'
            ) {
              prefix = 'NEWLINE';
            } else {
              prefix = 'SPACE';
        ...
        if (in_array(token_text.toLowerCase(), ['else', 'catch', 'finally'])) {
          ...
          if (
            ...
            if (
              (last_type !== 'TK_START_EXPR' || token_text !== 'var') &&
              last_text !== ':'
            ) {

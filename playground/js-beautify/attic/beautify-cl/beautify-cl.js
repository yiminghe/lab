...
function print_usage() {
  print(
    'Usage: java org.mozilla.javascript.tools.shell.Main beautify-cl.js [options] [file || URL]\n',
  );
  print('Reads from standard input if no file or URL is specified.\n');
  print('Options:');
  print('-i NUM\tIndent size (1 for TAB)');
  print(
    '-b [ collapse | expand | end-expand ]\t Put braces on the same line as control statements (default), or put braces on own line (Allman / ANSI style), or just put end braces on own line',
  );
    ...
    if (param.substr(0, 1) == '-') {
      switch (param) {
        ...
        case '-b':
          ...
          else options.brace_style = args.shift();
          break;
        case '-a':
...
function do_js_beautify() {
  ...
  var preserve_newlines = options.preserve_newlines

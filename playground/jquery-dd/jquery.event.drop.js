...
*/ (function ($) {
  ...
  $.fn.drop = function (fn1, fn2, fn3) {
    ...
    return !fn1
      ? this.trigger('drop') // 0 args
      : this.bind('drop', fn2 ? fn2 : fn1); // 1+ args
  };

  // DROP MANAGEMENT UTILITY
  $.dropManage = function (opts) {
    // return filtered drop target elements, cache their positions
    opts = opts || {};
  ...
  var $event = $.event,
    ...
    drop = ($special.drop = {
      ...
      $targets: $([]),
      data: [], // storage of drop targets and locations
      setup: function () {
        ...
        if (drop.data.length && event.dragTarget) {
          ...
          switch (event.type) {
            ...
            case 'mouseup': // DROP >> DROPEND >>
              ...
              if (drop.allowed) dropped = hijack(event, 'drop', drop.dropping); // trigger "drop"
              dropstart = false;
            ...
            case drop.allowed && 'dropstart': // DROPSTART >>
              ...
              drop.dropping =
                hijack(event, 'dropstart', this) !== false ? this : null; // trigger "dropstart"
              break;
          }
        }
      },
      ...
      locate: function (elem) {
        // return { L:left, R:right, T:top, B:bottom, H:height, W:width }
        var $el = $(elem),
          pos = $el.offset(),
          h = $el.outerHeight(),
          w = $el.outerWidth();
      ...
      contains: function (target, test) {
        // target { L,R,T,B,H,W } contains test [x,y] or { L,R,T,B,H,W }
        return (
          (test[0] || test.L) >= target.L &&
          (test[0] || test.R) <= target.R &&
          (test[1] || test.T) >= target.T &&
          (test[1] || test.B) <= target.B
        );
      },
      ...
      modes: {
        ...
        overlap: function (event, proxy, target) {
          ...
          target.overlap =
            Math.max(
              0,
              Math.min(target.B, proxy.B) - Math.max(target.T, proxy.T),
            ) *
            Math.max(
              0,
              Math.min(target.R, proxy.R) - Math.max(target.L, proxy.L),
            );
          if (target.overlap > ((this.best || {}).overlap || 0))
            // compare overlap
  ...
  function hijack(event, type, elem) {
    ...
    try {
      ...
      /* catch IE error with async event handling */
    }
  ...
  function tolerate() {
    ...
    do
      if ((drp = drop.data[i])) {
        // each drop target location
        // tolerance function is defined, or mouse contained
        winner = drop.tolerance
          ? drop.tolerance.call(drop, drop.event, drg, drp)
          : drop.contains(drp, xy)
          ? drp
          : null; // mouse is always fallback
      }
    while (++i < drop.data.length && !winner); // loop
    drop.event.type = (winner = winner || drop.best) ? 'dropstart' : 'dropend'; // start ? stop
    if (drop.event.type == 'dropend' || winner.elem != drop.dropping)
      // don't dropstart on active drop target

...
/* eslint no-new-wrappers: "off",
   ...
   var shellMethods = Object.create({});
   ...
   function wrap(cmd, fn, options) {
     ...
     return function () {
       ...
       try {
         ...
         state.pipedValue =
           this && typeof this.stdout === 'string' ? this.stdout : '';
         ...
         if (options.unix === false) {
           ...
           args = args.map(function (arg) {
             if (
               (typeof arg === 'string' && arg.slice(0, 2) === '~/') ||
               arg === '~'
             ) {
           ...
           if (!config.noglob && options.allowGlobbing === true) {
             args = args
               .slice(0, options.globStart)
               .concat(expand(args.slice(options.globStart)));
           }
